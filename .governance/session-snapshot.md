...
## 并行调度条件（用户 2026-09-14 纠偏：「与风险无关，是并行条件没搞明白」；下批直接适用）

**七条必要条件（每条可机械判定）**：① 写集不相交（`file_locks` 重叠 ⇒ 串行或 worktree 隔离）；② 无发生在先约束（B 的输入非 A 的输出 ⇒ 审查链 `R1→返工→R2` **天然串行，并行度 = 0**）；③ 共享基线冻结（契约/行号/HEAD 不得被任一分支改动；要改则须作为**第一个串行分支**——实例：UX-060 增行使 `host-contract` 硬行号必须重基）；④ 资源包络互斥（端口/浏览器 profile/DSH 实例/`%TEMP%`/模型配额各自独占）；⑤ **分支可落盘 = 并行单位 = 耐久单位**（不写盘的分支其存活期 = 会话存活期）；⑥ 验证独立（并行分支不得互审）；⑦ 编排容量（2~3 路为经验上限）。

**本会话调度失误（如实）**：CLEAN-004 自 R3 起的审查轮与定点返工**写集仅 `docs/**`**，与 BUG-009（`lib/client.js`/`lib/index.js`/`test/smoke.mjs`/`CHANGELOG.md`）**完全不相交且无 happens-before ⇒ 本可并行**；我却把它排在整条审查链之后，使 P2 产品缺陷在关键路径上空等数小时。**根因 = 把「审查链必须串行」（条件②）错误外推为「整个会话必须串行」**。

**修正后的调度规则**：先画依赖 DAG，只承认三类强制串行边（同文件写集 / 同共享基线 / has happens-before）；泳道按写集族切（产品代码 ∥ 验证资产与文档 ∥ 治理记录）；长链内部串行不可避免，但**用邻接泳道填满其等待期**；分支保持「小、可落盘、可中断」。

**重启语义（条件⑤的直接推论）**：本平台 subagent 是**进程内子 agent**（无独立服务；治理 SKILL 自述「C 级系统自动化未实现」）⇒ 重启杀死未落盘分支，已落盘者（commit/docs/快照/锁表）不受影响；跨会话续做靠**快照 + `agent-locks.json` + plan-tracker 任务行**外化状态，而非 agent 存活。
