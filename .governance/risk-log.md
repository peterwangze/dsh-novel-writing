...
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| RISK-001 | 2026-08-22 | CI 首跑曾失败（schemastery mock 无法覆盖 .default()/.union 链式 API），修复刚合入（fdf4edd），CI 稳定性待观察 | 后续提交的回归信号可信度下降；若 CI 再失败可能掩盖真实回归 | 中 | 中 | 观察最近 3 次 CI 运行结果；若再次失败，升级为 P0 任务根因分析 | 最近 3 次 CI 运行中出现非文档性失败 | Coordinator | 打开 | 2026-08-22 |
