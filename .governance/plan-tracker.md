...
| --- | --- | --- | --- | --- | --- | --- | --- |
| dsh-novel-writing | development (6/11) | 70 | 66 | 0 | 2 | G5 passed-with-conditions | — |
...
| **P1** | UX-037 | 左窗工作流面板修正（用户「调整比例，为何要二分？不应该根据内容调整吗」（删除标注指向误区——我误删门禁卡被纠正「删除的是阶段信息，下面的阶段图标已经承载阶段逻辑了啊，你乱删什么啊」））：①**删除「阶段信息卡」**（阶段: quality_review + 书名/题材/平台 + 章数/字数——冗余；阶段逻辑由清单 ✅🔄⬜ 图标承载）；②**门禁/清单卡恢复保留**（发布/变现/AI路径/偏离度 + 19 项阶段清单）；③左窗下段**内容自适应**（flex:0 0 auto + maxHeight 55%，取消固定 46% 二分——树占余量）；smoke（ux036 检查断言同步：46%→auto+55%、阶段信息卡负断言、门禁/请求卡正断言） | UX-036 | v0.4.0 | closed | ✅ 完成 (2026-08-28，EVD-053；node --check 通过；smoke 133/133；零宿主依赖保持) |
| **P1** | UX-038 | 左窗占比调整（用户「你调整布局的吗，我要的是工作流部分占比大一些」——UX-037 内容自适应后工作流占比偏小）：左窗分栏改为**工作流 60% / 文件树 40%**（workflow 容器 flex:0 0 60% + 内部滚动；树 flex:1 得余量 40% 可滚）；smoke（ux036 检查断言 60% 正断言） | UX-037 | v0.4.0 | closed | ✅ 完成 (2026-08-28，EVD-054；node --check 通过；smoke 133/133；零宿主依赖保持) |
...
| **P1** | UX-047 | 中窗上下工作台分隔线可拖（用户「把这个分割线也修改成可调节的，你的布局比例做的不行，就让用户有可操作的空间吧」）：新增 `.nv-middiv` 水平分隔线（4px row-resize 命中区 + hover/active accent 高亮）——**下半区高度 120px–60% bodyH 拖拽可调**（midDividerHandler pointer 捕获，拖起增大/拖下减小；默认 30%），**持久化 midH**（loadSplitSaved/persistSplit/novelSplit.setMidH 同构扩展）；i18n resizeMid 成对；smoke 133（新增 ux047 检查）；**修复**：snapshot() 缺 midH 致拖不动（EVD-063） | UX-046 | v0.4.0 | closed | ✅ 完成 (2026-08-28，EVD-062/063；node --check 通过；smoke 133/133；CDP 拖拽实测 166→120+持久化；零宿主依赖保持) |
| **P1** | UX-048 | 当前布局固化为默认（用户「把当前的布局作为默认布局吧」——非重置回内置值）：新增 **☆ 标题栏按钮「将当前布局设为默认」**——`novelSplit.saveAsDefaults()` 写**默认布局槽** `dsh.novel.defaults.v1`（leftW/chatW/chapterW/midH/chatSide），保存成功提示条（savedDefault）；`loadSplitSaved()` 无当前存档时**回退默认槽**（parseSplitSaved 抽取复用），再无槽→内置默认；**修复同款 bug**：open() 未回读 midH（持久化值重开丢失——现补 `this.midH = saved…`）；i18n saveAsDefault/savedDefault 成对；smoke 133（新增 ux048 检查；ux014 检查 nv-bar-ctl 计数 2→3） | UX-047 | v0.4.0 | closed | ✅ 完成 (2026-08-28，EVD-064；node --check 通过；smoke 133/133；零宿主依赖保持) |
...
| **P2** | UX-058 | ~~绑定弹窗分组头文案（用户复核反馈）~~ ⛔ 用户取消（2026-08-29「先保持不变吧，当前看起来也还行」）——未开工零代码改动；文案诉求留存备查（如后续重启按用户原话「当前工作区(N 个会话，点击展开)」） | UX-057 | v0.4.0 | cancelled | ⛔ 取消 (2026-08-29) |
...
| **P0** | BUG-004 | dsh 0.1.2-rc.1 客户端 API 表面断裂（用户 2026-09-07 实机报障三症状）：①dsh 0.1.2-rc.1 客户端 `connection` 服务已无 `.api` 表面（实证：仅 isLoopback/generation/state/rpc/reconnect/registerGenerationSource/start），API 迁移至 `remote.<namespace>` 服务（settings/session/workspace/agentPresets/directoryPicker）+ `workspaces`/`sessions` 服务快照 → 插件 client.js 全部 apiHas 检查失败 → 工作区对话框「API 不可用」/设置页 useEffect 内 TypeError 白屏/卡片打开·绑定·切换链全断；②sessions.current 跨工作区切换经 null 过渡被「就位/失联仅刷基准」守卫吞掉 → 点其他工作区会话不退出工作台（NvConsole L2424 与 SplitWorkspace L3075 两处）；③workspaceRoot 指向 C:\Users\peter\novels（已不存在——用户迁至 D:\AI\writing\novel-001\novel-project）→ 书目空。修复 = 客户端 API 适配层（新旧双表面特性检测）+ 会话联动 last-non-null 比较守卫 + manifest dsh.client.inject 包表核对（dsh-client-runtime 已不存在）；TRIAGE-BUG-004 五步 triage 通过（无阻塞无环；与 UX-054/058 文件重叠——均未在执行） | BUG-003 | v0.5.1 | closed | ✅ 完成 (2026-09-07，EVD-077/EVD-077a；commit 55da942——makeHostApi 适配层单点收口 44 调用点零改动 + last-non-null 守卫×2 + SettingsPage 防崩 + manifest 修正；smoke 147→179 全绿〔Coordinator 原生复跑〕；Code Reviewer R1 APPROVED_WITH_NOTES unresolved_blockers=0〔REVIEW-BUG-004-R1；F1 P2=smoke 错误分支断言真空待下轮顺手修，F2-F5 P3 非阻断〕；实机验收发现 BUG-005（症状 1 未消除——useSessions hook 面降级，探针实证）；workspaceRoot 重指 D:\AI\writing\novel-001 待用户执行） |

| **P0** | BUG-005 | useSessions React hook 面服务后到不自愈（BUG-004 实机验收发现）：用户完整重启 dsh web 后症状 1（点其他工作区 session 不退出）仍未消除——Coordinator 无头 Edge+CDP 探针活实例实证降级（控制台搜索无「找到的会话」section → sessionsAll=null → useSessionsH=null）；架构链：dsh 0.1.2-rc.1 sessions 服务由 dsh-api-session-controller 客户端提供（inject 含 remote.session——经 dsh-api-remotes 异步 $mount 链），本插件 apply 只等同步依赖（slots/connection/locale）→ L4298 ctx.get('sessions') 捕获 undefined → L4307 makeSessionsHook 冻结 null 永久降级；BUG-004 已修 launcher 面（internal/service 自愈 L4312-4321）但 hook 面标注「保持降级不中途翻转」（hook 调用计数安全顾虑）。修复 = 稳定响应式 hook：makeSessionsHook 改为返回恒定函数（服务经 holder 惰性解析 + internal/service 事件重订阅），服务缺席返回 undefined（消费端 ?? null/?? [] 已容错）；守卫逻辑（shouldCloseOnCurrentChange）不动。TRIAGE-BUG-005 五步 triage 通过（与 UX-054/058 文件重叠——均未在执行） | BUG-004 | v0.5.1 | closed | ✅ 完成 (2026-09-07，EVD-078/EVD-078a；commit 5fba438——makeSessionsHookReactive（恒定引用/惰性解析/重订阅/setServiceTick 扳机）+ 单一 internal/service 监听合并 + F1 顺带修；smoke 179→203 全绿〔Coordinator 原生复跑〕；Code Reviewer R1 APPROVED_WITH_NOTES unresolved_blockers=0〔REVIEW-BUG-005-R1〕；**隔离实例行为验证：点其他工作区会话→控制台自动关闭（症状 1 行为解除）+ 找到的会话 section 复活**；P2 遗留 6 项非阻塞——F1/F2 降级窗误报（各 1 行，建议并入下批）、F3 抽屉扳机覆盖（≤2.5s 有界）、F5 selector 重渲染、F6 状态点四态实三态（**验收口径**）、F9 变异痕迹已补录 EVD-078；待用户重启实机终验) |
