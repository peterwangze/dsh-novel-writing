...
| **P1** | REL-004 | 发布 v0.5.0（用户「发送版本承载修改」）：①CHANGELOG [Unreleased]→[0.5.0] 2026-08-30 ②package.json 0.4.0→0.5.0 ③git tag v0.5.0 推送 ④路线图 v0.5.0 行标已发布+里程碑更新 ⑤EVD-075 | UX-059 | v0.5.0 | closed | ✅ 完成（本地就绪）· ⏳ push 待网络恢复 (2026-08-30，EVD-075；发布前三件套全绿 0/29 PASS/147 0 failed；**commit ce047d0 + 本地 tag v0.5.0 已就绪**；推送被网络阻断——github.com:443 连接失败已重试 2 轮未恢复；网络恢复后 `git push origin main v0.5.0` 即完成；远程 ls-remote 实证待补录) |
