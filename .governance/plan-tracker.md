...
| --- | --- | --- | --- | --- | --- | --- | --- |
| dsh-novel-writing | development (6/11) | 52 | 47 | 0 | 2 | G5 passed-with-conditions | — |
...
| **P1** | UX-040 | 阶段徽标+统计信息迁移（用户「移到[工作流]并加上整体统计信息，例如当前字数信息等，也调整一下呈现的效果」——标题栏「quality_review ○」迁入左窗工作流面板并配整体统计）：①标题栏去除阶段徽标（.nv-badge）与绑定状态点（TitleDot）——标题栏仅保留书名（更简洁）；②WorkflowPanel 新增**阶段+统计卡**（呈现：本地化阶段名〔stages 映射，如「质量审查」〕+ 状态点（dot prop 传入 TitleDot）+ 《书名》+ `N 章 · M 字` 整体统计）；smoke（ux036 检查补 badge 负断言/TitleDot 恰一处/字数统计/阶段名映射断言） | UX-039 | v0.4.0 | closed | ✅ 完成 (2026-08-28，EVD-056；node --check 通过；smoke 133/133；零宿主依赖保持) |
