...
| ② 小说管理与人机高价值互动 | 宿主 `novel-writing` 服务管理 workspace 多书目；所有关键决策走 `ask_user_question`；工作台「请求队列」让人在 UI 侧发起优化/审查/发布/数据请求，协调者用 `novel_requests` 接单 |
| ③ 写作过程可视化 + 实时渲染 | 浏览器「小说」工作台（2s 轮询实时渲染正文/进度/门禁）+ 侧栏「小说 HUD」浮层 |
