...
- **▶ 开始工作流 / ▶ 继续工作流**：对当前书目把启动/续传指令直接注入对话（新书目自动判断为「开始」，有进度的为「继续」）。**预设自动保证**：会话已在小说预设 → 直接注入；会话还是空白 → 先自动切换到「小说写作工作流」预设再注入；会话已开始且用其他预设 → 给出出路提示。顶栏有实时预设状态徽标（✓ 已挂载 / ⟳ 将自动切换 / ⚠ 不匹配）；
- **默认预设**：**设置 → 小说写作 → 新会话默认预设**，一键把默认预设设为 `novel-writing`——此后每个新建会话自动就是小说工作流（已开始的会话预设锁定，不受影响）；
