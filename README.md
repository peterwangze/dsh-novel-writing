...
| 页签 | 用途 |
|---|---|
| 章节 | 左侧章节列表（**每行显示章节名**（`第N章 · 名称`，无章节名回退 `第N章`，不显示字数；列宽默认 160px、右缘分隔线可拖宽 120–360px 持久化，章节多时列表上下滚动——UX-019②③/UX-020）/已发布/门禁⚠）；中间阅读（场景分隔渲染，含「需求项」提示与机器审计折叠）或编辑（**Ctrl+S 保存**、未保存切换确认、保存跑门禁） |
