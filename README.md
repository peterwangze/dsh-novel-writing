...
| 页签 | 用途 |
|---|---|
| 章节 | 左侧章节列表（**每行显示章节名**（`第N章 · 名称 · M字`，无标题行回退仅字数；列宽默认 160px，拖右缘分隔线可调 120–360px 并记忆——UX-015②③）/已发布/门禁⚠）；中间阅读（场景分隔渲染，含「需求项」提示与机器审计折叠）或编辑（**Ctrl+S 保存**、未保存切换确认、保存跑门禁） |
