...
1. 安装并重启 DSH 后，**新会话初始窗口即出现「📖 小说工作台」入口条**（输入框上方）：点「▶ 开始/继续工作流」，或「＋ 新建小说」创建后立即开工——工作流预设自动按需挂载，全程无需手动选择预设（预设是工作台的附属组件，对用户不可见）；
2. 想先看看效果？把 [`examples/demo-novel`](./examples/demo-novel) 整个目录拷到 `~/novels/` 下，工作台即出现《示例小说》（含 1 章 + 看护卡，可体验阅读/编辑/门禁/发布面板）；
3. 小说默认存放在 `~/novels`（每部小说一个子目录；根目录可在**「小说」工作台侧栏顶部 ✎** 就地修改）；
