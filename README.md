...
| ③ 写作过程可视化 + 实时渲染 | **两级工作台**：侧栏「📖 小说管理工作台」区块（纯主入口：标题行开/关控制台 + 书目卡片）+ **管理控制台**（shell.overlay 全屏管理界面：仅管理工作区的创建/切换 + 小说的创建（＋磁贴）/删除（🗑 卡片钮）/卡片化呈现——排序（默认/手动拖拽）+ 卡片网格（书名/阶段/状态行/绑定双圆/数据摘要/图标操作 🔗🗑）、底部搜索小说/会话）；打开小说后进**小说创作工作台**（分栏工作区，标题栏「📖 小说创作工作台 ·《书名》」+ 阶段徽标——管理小说本身的创作与内容/状态/数据：margin 挤法把官方对话窗挤到右侧，左导航/中五页签/右对话，⇄ 换边、分隔线可拖、宽度记忆）；workspace 文件树（可折叠、点击预览）；2s 轮询实时渲染正文/进度/门禁 |
...
│   └─ 预设自动同步到 .agent-presets/        │    标题「📖 小说创作工作台 ·《书名》」｜左：书目+文件树 ｜ 中：五页签 ｜ 右：对话窗
...
> 每本小说与一个会话 **1:1 绑定**（settings `novel-writing.bindings`，overview 附带下发）：侧栏卡片点击 = 打开控制台并聚焦该书；控制台卡片点击 = 切换绑定会话并进**小说创作工作台**分栏（Esc/✕/再次点击标题行关闭控制台并还原布局）；删除小说（🗑）会先弹确认框，确认后删除整个书稿目录并自动清理该书绑定键。
...
1. 安装并重启 DSH 后，**侧栏底部出现「📖 小说管理工作台」区块（纯主入口）**：点**标题行**打开/关闭小说管理工作台控制台（反选语义），或点**书目卡片**直接打开控制台并聚焦该书；没有书目时显示「打开工作台界面创建第一本小说」；控制台打开期间**点击侧栏其他会话会自动退出工作台界面**（控制台随之关闭）；
...
4. 小说创作工作区 = 所选 DSH 工作区目录（其路径自动同步为小说 `workspaceRoot`）；此后可在**创作工作台（分栏）左窗顶部 ✎** 就地修改根目录（业务配置收敛于工作台）；
...
**小说管理工作台**（侧栏区块 = 纯主入口）：点标题行（或书目卡片 = 直接聚焦该书）打开**管理控制台**——全屏管理界面（覆盖内容区，不遮侧栏），只管三件事：**工作区的创建/切换、小说的创建（＋磁贴）与删除（🗑）、卡片化呈现**；标题行再点 / 控制台 ✕ / Esc 关闭；**控制台打开期间点击侧栏其他会话也会自动退出工作台界面**（自动关闭控制台，参照 dsh-worktable 项目×会话联动；插件自身链路——卡片打开/找到会话/新建——不受影响）：
...
**小说创作工作台**（打开小说后的分栏工作区）：管理小说本身的创作与内容/状态/数据——左窗（工作区根目录行 + 全部书目 + 当前书目 `novel-project/` 文件树，可「«」折叠）｜中窗（五个内容页签）｜右窗 = 官方对话窗本身（margin 挤法贴右，**⇄ 可换边到左侧**）；两条分隔线可拖拽调宽，宽度与换边记忆在本地（`dsh.novel.split.v1`）；标题栏：「📖 小说创作工作台 ·《书名》」+ 阶段徽标 · 绑定会话状态点 · **「▶ 开始工作流」（新书）/「▶ 继续工作流」（旧书）accent 实底钮**（UX-012：点击向绑定会话发送开始/继续指令；未绑定 → 提示并弹绑定面板，会话失效 → 提示重绑，会话服务缺席 → 提示暂不可用；busy 防连点）· ⇄ 换边 · ✕ 关闭（Esc 同）；
- **文件树**：当前书目 `novel-project/` 的完整文件树（可折叠、点击就地预览，.md 渲染阅读）。

> **配置边界**：业务配置统一在两级工作台内（工作区根目录 = 创作工作台分栏左窗顶部；平台发布模式/命令/数据抓取 = 发布页签「⚙ 平台发布配置」；小说↔会话绑定与删除 = 管理工作台控制台卡片 🔗/🗑）；**设置 → 小说写作**只保留插件级开关（启用/轮询/预设自动同步/LAN 暴露），与具体小说无关。
