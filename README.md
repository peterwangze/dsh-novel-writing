...
- **小说卡片网格**（玻璃拟态，minmax 320px / gap 20px——宽容器 3 列均分；卡片与「＋」磁贴统一 min-height 180px）：📖 书名 + 阶段徽标；状态行 `N章 · N字 · 发布✓/✗ · 变现✓/✗ · 信号N`；**绑定状态点**（双圆：○○ 空心未绑定 / ●● 实心已绑定 / 工作中=accent 蓝脉冲 / 待决=黄 / 完成=绿 / 会话失效=红）+ 最近更新时间；**数据卡片**：每书卡内一行最近 metrics 摘要（完读率/读完率/追读/日增收藏/收益，无数据显示「暂无数据」）；右下角两个 22×22 图标钮（🔗 绑定·重绑 / 🗑 删除）；书目 >8 本时折叠为前 8 + 「展开全部（N）／折叠」行；
