...
- **结论**：**有条件通过（Go —— 本地发布）**；条件 = ①push 未执行（由 Coordinator 按执行序执行）；②2 项门禁工具**不可用**（BLOCKED-by-upstream，见 §H，**未包装为 PASS**）
...
| U3 | `check-release` → `loop runtime claim gate` | 6 条（3× `AUTHORITY_SOURCE_OCCURRENCE`、1× `AMBIGUOUS_SUBJECT_RELATION`、3× `UNSUPPORTED_AFFIRMATIVE`，均点名插件仓 `docs/reviews/review-FIX-300-CODE-R0.md`、`docs/release/release-checklist-0.81.0.md`） | 复核面为**插件仓文档** |
...
| U6 | `check-release` → `release docs`（3 条） | `docs\release\{release-checklist,feature-flags,rollback-plan}-0.5.3.md: missing …` | `verify_workflow.py` **L6251-6262**：`def check_release_docs_coverage(version, root=None): root = Path(root) if root is not None else ROOT`，而**调用点 L7047 为 `check_release_docs_coverage(version)`——不传 root** ⇒ `root = ROOT` = **插件仓根**。实测对照：插件仓 `docs/release/` 为其自身 0.31.0~0.81.0 产品线（**无** `*-0.5.3.md`），宿主仓**有**三件 `*-0.5.3.md` ⇒ **该子门禁校验的是插件自身版本线，对任何宿主项目必然为红**。与 U1 同源（跨根期望源） |

### H.2 宿主侧真实缺失（逐条列出与处置）

| # | 项 | 实测 | 处置 |
| --- | --- | --- | --- |
| H1 | `check-release` → `release fact source` | `.governance/plan-tracker.md`：缺 `1.0.0 依赖链` 节、缺 `1.0.0 roadmap row`、需求矩阵缺 `REQ-059`~`REQ-064`（共 9 项中的 8 项） | ⚠️ **宿主治理记录结构性缺口**，位于 `.governance/**` = **REL-007 写边界外**（本任务被明确禁止写该目录）⇒ **升级 Coordinator 处置**，本版不修、不掩盖 |
| H2 | `check-release` → `execution gates` → `governance health` | exit 1；直跑 `check-governance --fail-on-issues` = **16 issue(s)**：`BLOCKING` 5 条（4× `REL-007 task_not_in_plan`〔Coordinator 已补 plan-tracker 行〕+ 1× `CLEAN-006` 锁过期 TTL）、`V2` 轮次连续性遗留提示 10 条、`M5.4b` evidence-log 散文前缀 1 条 | ⚠️ 除 REL-007 登记缺失（**已由 Coordinator 修复**）外**均为先前存在**状态；`.governance/**` 属写边界外 ⇒ 升级 Coordinator |
| H3 | 残留计数的口径差异 | `check-release` 内 `governance health` 汇总报 `88 issue(s)`，而直跑同一命令报 `16` | 如实登记为**口径差异**（前者经 check-release 聚合口径/doctor 面），**不解读为新增缺陷**；两读数均来自本清单 §G 的实跑 |
...
### H.3 宿主侧可自证的替代判据集合（**不构成 U1/U5 的替代 PASS**）
...
> **诚实声明**：`stage-release` SKILL 退出条件含「候选态 `check-release` PASS」与「declarative manifest 闭环」——本次**均未满足**（U1~U5 工具缺陷，非本版交付缺陷）。本清单**不主张**该两条通过：状态维持 `FAILED — 22 issue(s)` 与 `BLOCKED（崩溃）`，处置归 Coordinator（RISK/SYSGAP 登记）。
