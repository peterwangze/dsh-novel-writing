...
- **发布类型**：本地发布（发布提交 + 本地 annotated tag，**未 push**；**push 已授权**（DEC-027，用户已授予 `workflow` scope），按执行序待 R2/R3 审查通过后由 Coordinator 执行）
- **发布提交**：`e1f25df84ead0a4d07c655f9afcd379a8a96a2a8`（版本 bump）+ 返工链 `792030c`（R1 补强）→ `6f0027b`（M7.7 事件机写证据）→ `914725f8`（演练工件固化）；**tag**：`v0.5.2`（对象 **`793dde78…` → peel `914725f8…`**〔DEC-027 重定后，= 返工链末端证据固化提交〕）
- **tag 身份 supersede 留痕**：重定前旧对象 `7ca99d6d…` → peel `e1f25df8…` **已随 DEC-027 重定删除**（`git for-each-ref refs/tags/v0.5.2` 不再返回）——记录值以本行新身份为准
- **结论**：**有条件通过（Go —— 本地发布）**；条件 = ①push 已授权（DEC-027，用户已授予 `workflow` scope）——按执行序待 R2/R3 审查通过后由 Coordinator 执行；②1 项门禁工具不可用已升级（见 §G）。
...
| 决策 | 结论 | 理由 |
| --- | --- | --- |
| 可以发布 / 有条件发布 / 暂缓 | **有条件发布（Go —— 本地）** | 范围分层正确 + 三件套全绿 + 版本三方一致；条件 = ①push **已授权**（DEC-027，用户已授予 `workflow` scope；RISK-004 已进入「待执行」）——按执行序待 R2/R3 审查通过后由 Coordinator 执行；②`check-release` 门禁不可用（§G）已升级裁定 |
