...
// ══════════════════════════════════════════════════════════════════════════════
// 【R2 N-02 修复】共享谓词登记表 —— 部署断言与可失败性自证**唯一同源**
//
// 与自证方的**唯一合法接口**：① 部署断言经 `evalPred(<id>, 测量值)` 消费本表的 `fn`；
// ② 自证方（本文件 `falsifiabilityReport()` 与 `falsifiability-check.mjs`）从本块
// 提取 `PREDICATE_REGISTRY` 后对 `red*`/`ok*` 向量求值。
//
// 纪律（由 `falsifiability-check.mjs` 机检，违反即 EXIT 1）：
//   · 每条谓词的**全部判定语义**只允许写在本块内（纯函数，输入 = 测量值对象，输出 = boolean）；
//   · 部署侧不得就地重写同一谓词的表达式——只允许 `evalPred('<id>', {...})` 取值；
//   · 自证方 MUST 对每条登记谓词至少求值 1 个 red（⇒ false）与 1 个 ok（⇒ true）向量——
//     向量被静默丢弃（R2 指出 `red3` 被 `falsifiabilityReport()` 漏读）即视为自证失效。
//
// 每条：`fn` 纯函数 · `red*` 反例向量（真实缺陷形态 ⇒ MUST false）· `ok*` 正例向量（MUST true）。
// ══════════════════════════════════════════════════════════════════════════════
const PREDICATE_REGISTRY = [
  ...
  },
  {
    ...
    finding: 'F-03 → R2 N-01/N-03 重设计',
    /**
     * 前提重设计（R2 N-01/N-03）：原实现以固定关键词 `孤星` 检索，而该串与
     * `displayTitleOf = title ?? cwd 名 ?? id`（宿主 `lib/client.js:2996-3004`）无交集
     * （插件 `sessions.create` 不传 title）⇒ 空结果为**预期行为**，不是缺陷。
     * 新前提 = `effectiveKeyword`：宿主已认可 id 的 **UI 展示串**（`session-<uuid>`，与宿主
     * `displayTitleOf` 的 id 兜底逐字一致）；镜像体可读时以其实测 `displayTitle` 校验——
     * 两者都是**实际会出现在 `displayTitle ?? id` 中的字符串**，检索前提必然可达。
     * 判定 = 「检索前提在场（`kwOk`）⇒ 检索 MUST 渲染 ≥1 行且行标题可核」。
     * **红通道（可失败）**：检索 0 行 = 镜像未反映宿主会话 / 消费面失效 / 订阅失效。
     * 注：**不再**把「镜像体（`sessionsAll` 快照对象）字段读数」写成谓词分量——部署侧无法从 DOM
     * 读出该内部对象（见 `facts.mirrorBodyUnreachable`）；该残余不确定性在 README §10 声明。
     * 反例向量：`red`（检索 0 行）· `red2`（行标题缺失）· `red3`（检索前提缺失 ⇒ 不可判绿）。
     */
    fn: (v) => v.kwOk === true && v.rows >= 1 && v.titleOk === true && v.effectiveKeyword !== null,
    red: { note: '宿主已认可该会话 ∧ 检索 0 行 ⇒ 镜像/消费面未反映宿主会话（D-3 主张的原形）', v: { ackComplete: true, kwOk: true, effectiveKeyword: 'cf54dd', ids: [], displayTitle: null, rows: 0, titleOk: false } },
    red2: { note: '有行但行标题缺失（渲染面损坏）', v: { ackComplete: true, kwOk: true, effectiveKeyword: 'cf54dd', ids: [], rows: 1, rowTitles: [null], titleOk: false } },
    red3: { note: '检索前提缺失（关键词不可得）∧ 0 行 ⇒ 不可判为绿', v: { ackComplete: true, kwOk: false, effectiveKeyword: null, ids: [], rows: 0, titleOk: false } },
    ok: { note: '关键词命中 ⇒ 渲染 ≥1 行且行标题可核（记录运行实测形态：1 行）', v: { ackComplete: true, kwOk: true, effectiveKeyword: 'cf54dd', ids: [], displayTitle: null, rows: 1, rowTitles: ['novels'], titleOk: true } },
    // 注：**无** «宿主未认可 ⇒ N-A» 的正例向量——N-A 由部署断言的 `status` 门控（`ackComplete`）承担，
    // 不是谓词的分量（`ackComplete` 已从谓词中移出，见上）。谓词只对「取得观测后 MUST 命中」负责。
  },
  {
    id: 'B21b-search-find-sessions-retest',
    finding: 'F-03 → R2 N-01/N-03 重设计',
    // 与主链同语义（同一谓词实例），前置换成 §16 的收尾复测：控制台已在场且关键词已注入。
    fn: (v) => v.consoleOpen === true && v.kwOk === true && v.rows >= 1
      && v.titleOk === true && v.effectiveKeyword !== null,
    red: { note: '控制台在场 ∧ 检索 0 行（复测同果）', v: { ackComplete: true, consoleOpen: true, kwOk: true, effectiveKeyword: 'cf54dd', ids: [], rows: 0, titleOk: false } },
    red2: { note: '检索前提缺失（关键词不可得）∧ 0 行 ⇒ 不可判绿', v: { ackComplete: true, consoleOpen: true, kwOk: false, effectiveKeyword: null, ids: [], rows: 0, titleOk: false } },
    red3: { note: '控制台未取得 ⇒ 该分支由断言显式记 N-A；若控制台缺席却仍按 PASS 口径评估 ⇒ 谓词必须为 false', v: { ackComplete: true, consoleOpen: false, kwOk: true, effectiveKeyword: 'cf54dd', ids: [], rows: 1, titleOk: true } },
    ok: { note: '控制台在场 ∧ 关键词命中并渲染', v: { ackComplete: true, consoleOpen: true, kwOk: true, effectiveKeyword: 'cf54dd', ids: [], rows: 1, rowTitles: ['novels'], titleOk: true } },
    // 注：`consoleOpen !== true` 的 N-A 由部署断言的 `status` 门控承担（非谓词分量）。
  },
  {
  ...
  },
  {
    ...
    /**
     * R2 N-04 附带订正：原谓词末项为 `h <= 32`（**像素高度阈值**）——记录运行实测 `form=row / 单行 /
     * h=29 / 无溢出` 三项本已可失败，但像素项对「内容未截断」只是**代理量**。现改判**测量项**：
     *   · `overflow.sw <= overflow.cw + 1`：未横向截断（保留原项）；
     *   · `overflow.sh <= overflow.ch + 1`：未纵向截断（**新增**）。
     * `overflow.sh/ch` 由部署侧**逐后代取最大值**（`max(scrollHeight)` vs `max(clientHeight)`）——
     * 只量载体自身恒等（块级元素自身不滚动）⇒ 必须扫描后代才具区分力（详见部署侧采集代码）。
     * 像素阈值不再是判定项（仍在 detail 中作为读数）。反例向量：block / 缺席 / 横向溢出 / 纵向截断。
     */
    fn: (v) => v.present === true && v.form === 'row' && v.lines === 1 && v.titleAttr === ''
      && v.overflow.sw <= v.overflow.cw + 1 && v.overflow.sh <= v.overflow.ch + 1,
    red: { note: '短提示被渲染成 block（当长告警处理）', v: { present: true, form: 'block', lines: 3, titleAttr: '', overflow: { sw: 558, cw: 558, sh: 60, ch: 60 } } },
    red2: { note: '短提示不在场（提示通道未渲染）', v: { present: false, form: null, lines: null, titleAttr: '', overflow: { sw: 0, cw: 0, sh: 0, ch: 0 } } },
    red3: { note: '短提示单行但横向溢出被截断', v: { present: true, form: 'row', lines: 1, titleAttr: '', overflow: { sw: 700, cw: 558, sh: 29, ch: 29 } } },
    red4: { note: '短提示单行但载体纵向截断（内容高于载体）', v: { present: true, form: 'row', lines: 1, titleAttr: '', overflow: { sw: 558, cw: 558, sh: 44, ch: 29 } } },
    ok: { note: '本轮实测形态：row / 单行 / 无溢出 / 无容器截断', v: { present: true, form: 'row', lines: 1, titleAttr: '', overflow: { sw: 558, cw: 558, sh: 29, ch: 29 } } },
  },
  {
    id: 'C11-flip-back',
...
// ══════════════════════════════════════════════════════════════════════════════
/**
 * R2 N-02 修复：可失败性自证**消费共享登记表**（`PREDICATE_REGISTRY`）。
 *
 * 与 R1 版本的三处差别（对应 R2 的具体指控）：
 *   ① 向量**全量求值**：不再写死 `red`/`red2`+`ok`/`ok2` 四个键——按 `<prefix>\d*` 通配收集，
 *      `red3`/`red4` 不再被静默丢弃（R2：`falsifiabilityReport()` 只读 `red`/`red2`）；
 *   ② 每条谓词 MUST ≥1 red 与 ≥1 ok 向量，否则 `pass=false`（向量缺失即自证失效，不得静默通过）；
 *   ③ 附 `deployments`：逐条登记**部署断言对该谓词的消费点**（`evalPred('<id>', …)` 计数 +
 *      谓词源文本 sha256）⇒「部署面 = 自证面」在报告内可核（非同源立即显形）。
 *
 * @param {Record<string,{count:number,sha:string}>} [deployments] 运行期累计的部署消费点。
 * @returns {{rows:object[],allPass:boolean,vectorTally:{red:number,ok:number}}} 逐条结果。
 */
function falsifiabilityReport(deployments, staticSites) {
  const rows = []
  const keys = (p, prefix) => Object.keys(p).filter((k) => new RegExp('^' + prefix + '\\d*$').test(k) && p[k] !== undefined)
  let redTotal = 0
  let okTotal = 0
  for (const p of PREDICATE_REGISTRY) {
    const redResults = keys(p, 'red').map((k) => ({ vector: k, note: p[k].note, result: p.fn(p[k].v) }))
    const okResults = keys(p, 'ok').map((k) => ({ vector: k, note: p[k].note, result: p.fn(p[k].v) }))
    redTotal += redResults.length
    okTotal += okResults.length
    const dep = deployments === undefined ? null : (deployments[p.id] ?? null)
    // 静态绑定（源码 `evalPred('<id>'` 调用点数）≠ 运行期消费（该分支是否被走到）：
    // `C-face-availability` 的例子——本轮闸门未进入 ⇒ 运行期 0 消费，但源码绑定存在且谓词已入自证。
    const sites = staticSites === undefined ? null : (staticSites[p.id] ?? 0)
    rows.push({
      id: p.id, finding: p.finding,
      fnSha256: sha256(String(p.fn)),
      redCount: redResults.length, okCount: okResults.length,
      deployment: {
        staticCallSites: sites, staticBound: sites !== null && sites >= 1,
        runtimeCallSites: dep === null ? 0 : dep.count, fnSha256: dep === null ? null : dep.sha,
        consumed: dep !== null && dep.count >= 1,
      },
...
/**
 * **源码级**部署绑定检查（R2 N-02 的核心）：统计本文件内 `evalPred('<id>'` 的**静态调用点**数。
 * 为什么需要它：运行期 `predicateTake` 只记「被执行到的」分支——闸门未进入的谓词（如
 * `C-face-availability`）运行期计数恒 0，**不能**据此判「未绑定」。同源性的机证必须落在源码面：
 * 登记表每条谓词 MUST 有 ≥1 处 `evalPred` 消费点（`falsifiability-check.mjs` 独立复核同一事实）。
 *
 * @returns {Record<string, number>} 逐谓词的静态调用点计数。
 */
function staticCallSiteCounts() {
  const src = readFileSync(new URL(import.meta.url), 'utf8')
  const out = {}
  for (const p of PREDICATE_REGISTRY) {
    out[p.id] = src.split("evalPred('" + p.id + "'").length - 1
  }
  return out
}

/** 运行期部署消费点台账（`FALSIFIABILITY-PROOF` 断言的 `deployments` 面）。 */
const predicateTake = {}

/**
 * **部署侧唯一合法判定入口**（R2 N-02 同源要求）：按 id 取共享登记表的谓词函数实例并求值。
 * 部署断言 MUST NOT 就地重写谓词表达式——`falsifiability-check.mjs` 会机检
 * 「登记表每条谓词都有 `evalPred('<id>', …)` 消费点」（缺失或改写即 EXIT 1）。
 *
 * @param {string} id 登记表内的谓词 id。
 * @param {object} measurement 测量值对象（键与登记表 `red*`/`ok*` 向量同形）。
...
async function main() {
  ...
  try {
    ...
    // R2 N-02 修复：判定经共享登记表（`evalPred`）——表达式的唯一副本在 `PREDICATE_REGISTRY`，
    // 自证方对同一函数实例求反例向量 ⇒「构造反例 ⇒ 必红」覆盖真正上场的断言。
    const b5Predicate = evalPred('B5-found-sessions-area', { hostHasRows: b5HostHasRows, foundTitle: foundState.foundTitle, rows: foundState.rows.length })
    assertion('B5-found-sessions-area', '控制台/搜索', '「找到的会话」区 = 宿主侧栏确有会话行时**必渲染**（判据：宿主会话行数 ≥1 ⇒ 本区存在性必须一致；谓词取自共享登记表 `B5-found-sessions-area`，无恒真分支）',
      b5Predicate,
      { kw: foundState.kw, foundTitle: foundState.foundTitle, rows: foundState.rows.length, hostSidebarRows: hostSidebarAtB5, legacyHostSidebarText: foundState.hostSidebar, b5HostHasRows, b5MirrorRendered, predicateId: 'B5-found-sessions-area' },
    ...
    assertion('B12b-tile-min-height', '控制台/网格', '＋磁贴与卡片统一最小高度 180px（卡内含内容时可更高——几何下限判据；空集显式判红：`Math.min()` 空数组 = Infinity；谓词取自共享登记表 `B12b-tile-min-height`）',
      evalPred('B12b-tile-min-height', { tileH: gridState.tile === null ? null : gridState.tile.h, cardHeights: gridState.cardHeights }),
      { tileH: gridState.tile === null ? null : gridState.tile.h, cardHeights: gridState.cardHeights, emptySet: gridState.cardHeights.length === 0, predicateId: 'B12b-tile-min-height' })
    ...
    cFaceRest: {
      if (panels.splitClosed === true) {
        ...
        // R2 N-02：判定经共享登记表；N-A 门控与谓词**互斥**（异常态绝不被 N-A 吞掉）。
        const cFacePredicate = evalPred('C-face-availability', { splitOpened, splitClosed: panels.splitClosed === true })
        assertion('C-face-availability', '分栏/存活', 'C 面存活闸门：C1 已观测分栏打开（`.nv-bar` 在场）而 C5 时刻分栏不在场 ⇒ 分栏「打开后自发关闭」为回归异常（FAIL）；从未打开过才记 N-A（谓词取自共享登记表 `C-face-availability`）',
          cFacePredicate, { ...report.facts.cFaceGate, predicateId: 'C-face-availability' },
          cFaceAnomaly === true ? undefined : 'N-A')
        const notRun = [
    ...
    assertion('C11-flip-back', '分栏/换边', '⇄ 再点 → 回到 chatSide=right（两态可逆；须先确认 ⇄ 确实被点击；谓词取自共享登记表 `C11-flip-back`）',
      evalPred('C11-flip-back', { flipBackClick, chatSide: sideBack === null ? null : sideBack.chatSide, splitOpen: sideBackSplitOpen }),
      { flipBackClick, sideBack, sideBackSplitOpen, predicateId: 'C11-flip-back' },
    ...
    const prepareSplitForNotice = async () => {
      const attempts = []
      /**
       * R2 N-06 修复：原实现直接进入构造，未处理 `§10 C9` 失败重开遗留的 `.nv-modal` 绑定面板
       * ——两轮记录均显示 `attempts[0].bindPanel=true`，即 E/F/D3 的观测发生在**非干净 DOM 态**
       * （遗留模态在场）而报告未披露。现改为：进入构造前**先收敛遗留遮罩**并落证：
       *   · 短暂等待（8 s）让可能的自动关闭发生；
       *   · 仍在 ⇒ 点遮罩关闭（产品既有路径，`facts.reopenDiag.bindPanel` 的同一手法）；
       *   · 结果写入 `report.facts.prepOverlayClean`（而非塞进 `attempts`，避免与「卡点击尝试」混淆）。
       * 遗留遮罩若无法关闭 ⇒ 记录 `ok:false`（构造前置为脏态）但仍继续尝试，由各断言自身的前置判据决定 N-A/FAIL。
       */
      const overlayPresentBefore = await cdp.evaluate("document.querySelector('.nv-modal') !== null")
      let overlayClosedBy = overlayPresentBefore === true ? null : 'absent'
      let overlayPresentAfter = overlayPresentBefore
      if (overlayPresentBefore === true) {
        for (let i = 0; i < 16 && (await cdp.evaluate("document.querySelector('.nv-modal') !== null")) === true; i += 1) await sleep(500)
        overlayPresentAfter = await cdp.evaluate("document.querySelector('.nv-modal') !== null")
        if (overlayPresentAfter === true) {
          overlayClosedBy = 'backdrop-click'
          await cdp.evaluate("(() => { const bd = document.querySelector('.nv-modal-backdrop'); if (bd !== null) bd.click(); return bd !== null })()")
          await sleep(900)
          overlayPresentAfter = await cdp.evaluate("document.querySelector('.nv-modal') !== null")
        } else {
          overlayClosedBy = 'auto-closed'
        }
      }
      const prepOverlayClean = {
        initialModal: overlayPresentBefore, closedBy: overlayClosedBy, finalModal: overlayPresentAfter,
        ok: overlayPresentAfter === false,
        note: 'R2 N-06：构造 E/F/D3 前置前的遗留遮罩（.nv-modal）收敛记录——finalModal=false 表示观测在干净 DOM 态进行',
      }
      if (report.facts.prepOverlayClean === undefined) report.facts.prepOverlayClean = []
      report.facts.prepOverlayClean.push(prepOverlayClean)
      ...
      if (splitNow === true) return { ok: true, attempts, reason: 'already-open', prepOverlayClean }
    ...
    const shortState = await cdp.evaluate(`(() => {
      ...
      return {
        ...
        // 纵向溢出必须**逐后代**取最大值：块级载体自身不滚动（sh ≡ ch）⇒ 只量自身恒真、
        // 对「内容被裁掉」无区分力（R2 N-04 的同类问题）。任一后代内容超出自身盒 ⇒ 计为截断。
        overflow: (() => {
          const nodes = [note, ...note.querySelectorAll('*')]
          let sh = 0, ch = 0
          for (const n of nodes) { sh = Math.max(sh, n.scrollHeight || 0); ch = Math.max(ch, n.clientHeight || 0) }
          return { sw: note.scrollWidth, cw: note.clientWidth, sh, ch }
        })(),
      }
    })()`)
    ...
    /**
     * R1 F-04 后续订正 + R2 N-04 附带订正：UX-060 的**契约**是「长告警 → in-flow 流式面；
     * 短提示 → 紧凑单行」。实测短 ok 提示走 `.nv-notice[data-form=row]`（`lib/client.js:L4177`：
     * 流式面自身按文本长度区分 `row`/`block` 形态），即「紧凑单行承载」而非标题栏内 `.nv-bar-note` chip。
     *   · 判定项 = 在场 ∧ `data-form=row` ∧ 单行 ∧ `title` 置空 ∧ **无溢出**（含纵向）；
     *   · `h <= 32` 像素阈值**不再是判定项**（R2：对「内容未截断」是代理量）——仅作 detail 读数。
     * 反例自证（共享登记表 `E1-short-notice-compact` 的 4 个 red 向量）：block 形态 / 提示缺席 /
     * 横向溢出 / 载体纵向截断 ⇒ 任一即红。所有判定经 `evalPred` 消费同一谓词实例。
     */
    const shortMeasured = shortState === null
      ? null
      : {
          present: true, form: shortState.form, lines: shortState.lines, titleAttr: shortState.titleAttr,
          overflow: { sw: shortState.overflow.sw, cw: shortState.overflow.cw, sh: shortState.overflow.sh, ch: shortState.overflow.ch },
        }
    const shortIsCompact = evalPred('E1-short-notice-compact', shortMeasured ?? { present: false, form: null, lines: null, titleAttr: '', overflow: { sw: 0, cw: 0, sh: 0, ch: 0 } })
    assertion('E1-short-notice-compact', 'notice', '短提示（≤24 字符，`NOTICE_INLINE_MAX`）走紧凑单行承载：在场 ∧ data-form=row ∧ 单行 ∧ title 置空 ∧ 无横向/纵向溢出（`title` 置空 = 不靠 tooltip 兜底；像素读数 `h` 仅记录不判定）——UX-060 短提示零变化；触发 = 幂等动作（恢复/设为默认）；谓词取自共享登记表 `E1-short-notice-compact`',
      shortIsCompact === true, { shortState, restoreClicked, shortIsCompact, shortMeasured, predicateId: 'E1-short-notice-compact', hThresholdNote: 'R2 N-04：h<=32 已降为非判定读数' })
    ...
    assertion('D3-mutual-exclusion', '控制台/分栏', '控制台与分栏互斥：分栏**先在场**，点抽屉标题行开控制台 → 分栏被这次动作关掉（DEC-015；前置不成立则记 N-A；谓词取自共享登记表 `D3-mutual-exclusion`）',
      evalPred('D3-mutual-exclusion', { barOpenBefore: exclusivity.barOpenBefore, consoleOpen: exclusivity.consoleOpen, barOpenAfter: exclusivity.barOpenAfter }),
      { ...exclusivity, d3Prep, predicateId: 'D3-mutual-exclusion' },
    ...
    /**
     * R2 N-08 修复：原 `degradedBranchReachable: false` 是**硬编码常量**却与实测字段并列写在 detail 里，
     * 读者会误读为「已实测确认降级分支不可达」。现拆成两项：
     *   · `degradedBranchReachable_analytic`：**分析值**（常量）——隔离平面必然提供 `sessions` 服务
     *     （`useSessions !== null`），故产品自身的 `degraded` 分支在隔离实例不可达（归口 `checklist §9 R-03`）；
     *   · `degradedBranchReachable_measured=false`：**实测值**——本运行未观测到任何缺席信号
     *     （既无双圆也无降级 chip / 无「会话服务不可用」文案），即「缺席分支未被实际走到」。
     * 说明其来源的口径同时写入 README §10；判定仍以共享登记表 `D5-sessions-present` 为准。
     */
    const degradedReach = {
      degradedBranchReachable_analytic: false,
      degradedBranchReachable_analytic_source: '分析：隔离平面的宿主必然提供 sessions 服务（useSessions!==null）⇒ 产品 degraded 分支不可达；归口 checklist §9 R-03 / U-12',
      degradedBranchReachable_measured: serviceAbsent === true,
      degradedBranchReachable_measured_source: '实测：本运行未观测到任何「服务缺席」信号（degradedProbe 的 dot/chip/innerText 三面皆无缺席线索）',
    }
    assertion('D5-sessions-present', '降级态', '会话服务可用性以产品自身 `degraded` 派生渲染面为准（L3148/L2893/L4034）：服务可用 ⇒ 抽屉双圆与控制台双圆**同时**在场；二者皆无 ⇒ 服务缺席分支（本实例该分支不可达 ⇒ 记 N-A，不冒充可判别 PASS）；谓词取自共享登记表 `D5-sessions-present`',
      evalPred('D5-sessions-present', { serviceSignal, serviceAbsent, drawerDots: degradedProbe.drawerDots, consoleDots: degradedProbe.consoleDots, consoleCards: degradedProbe.consoleCards }),
      { ...degradedProbe, serviceSignal, serviceAbsent, ...degradedReach, legacyPredicate: gridState.counts.cdot >= 1, predicateId: 'D5-sessions-present' },
      serviceSignal === true ? undefined : 'N-A')

    // ══ 会话数据面断言 R2 重构（N-01 + N-03）════════════════════════════════════════
    // 契约（`lib/client.js:L3394-3397`）：`foundSessions = ids.map(id => byId[id]).filter(e =>
    //   String(e.displayTitle ?? e.id).toLowerCase().includes(kwL) || String(e.id).toLowerCase().includes(kwL))`
    // ⇒ 检索关键词 MUST 是「实际会出现在 `displayTitle ?? id` 中的字符串」才有落点。
    //
    // R2 已证：原固定关键词「孤星」与 `displayTitleOf = title ?? cwd 名 ?? id`（宿主 L2996-3004）
    // **无交集**（插件 `sessions.create` 不传 title，实测 id 为随机 UUID）⇒ 空结果是**预期行为**，
    // 据此判「镜像未反映宿主会话」属**前提不可达**的假阳性（N-03：D-3 撤回）。
    //
    // 本轮前提重设计（N-01 的 (a) 分支）：
    //   · `mirrorBody` 是**镜像本体读数**（`ids` + 每条 `byId`）——不再由「搜索结果空」反推「镜像空」；
    //   · `effectiveKeyword` 逐级取：宿主已认可会话 id 的 **UI 展示串**（前缀 `session-`，与
    //     `displayTitleOf` 的 id 兜底逐字一致）→ 镜像实测 `displayTitle` 中的实词 → 完整 id 兜底；
    //   · 判定 = 前提在场（`kwOk`）⇒ 镜像 MUST 渲染 ≥1 行且行标题可核（红通道：镜像空 / 0 行 / 标题缺失）；
    //   · 镜像**确为空**（`ids.length === 0`）时谓词求值为 false ⇒ **FAIL**（而非掩盖为 N-A）。
    const mirrorBody = await cdp.evaluate(`(() => {
      // 采样点 1：主文档；采样点 2：每个**同源** iframe 的 document（DSH 外壳可能把应用放在 iframe 内）。
      const realms = [document]
      try { for (const f of document.querySelectorAll('iframe')) { try { if (f.contentDocument !== null) realms.push(f.contentDocument) } catch (e) { /* cross-origin */ } } } catch (e) { /* ignore */ }
      const found = []
      let scanned = 0
      for (const doc of realms) {
        let nodes = []
        try { nodes = [...doc.querySelectorAll('*')].slice(0, 4000) } catch (e) { nodes = [] }
        for (const n of nodes) {
          scanned += 1
          let keys = []
          try { keys = Object.keys(n) } catch (e) { keys = [] }
          for (const k of keys) {
            let v = null
            try { v = n[k] } catch (e) { continue }
            if (v !== null && v !== undefined && typeof v === 'object' && Array.isArray(v.ids) && v.byId !== null && v.byId !== undefined) {
              found.push({ ids: v.ids.slice(0, 20), byId: Object.fromEntries(v.ids.slice(0, 20).map((id) => [id, { displayTitle: (v.byId[id] ?? {}).displayTitle ?? null, agentPreset: (v.byId[id] ?? {}).agentPreset ?? null }])) })
            }
          }
        }
      }
      if (found.length === 0) return { reachable: false, reason: 'no-mirror-node-found', realms: realms.length, scanned }
      const body = found[0]
      return { reachable: true, realms: realms.length, scanned, ids: body.ids, byId: body.byId }
    })()`)
    report.facts.mirrorBody = mirrorBody
    if (mirrorBody.reachable !== true) report.facts.mirrorBodyUnreachable = { reason: mirrorBody.reason ?? null, realms: mirrorBody.realms ?? null, scanned: mirrorBody.scanned ?? null, note: '镜像体（sessionsAll 快照对象）不在任何 DOM 节点属性上 ⇒ 本体读数不可得；判定改以**渲染面**（搜索命中行）为准，并在 README §10 声明该残余不确定性' }

    // 宿主认可面（§14 与 §16 同一口径）：overview 绑定 id ∧ 宿主工作区表 sessionIds ∧ 隔离盘会话存储
    ...
    /** 宿主已认可会话在 UI 中的稳定展示串：`session-<uuid>`（与宿主 `displayTitleOf` 的 id 兜底同形）。 */
    const hostApprovedUiId = (() => {
      const raw = typeof acked.boundId === 'string' && acked.boundId.length > 0 ? acked.boundId : (expectedIds[0] ?? null)
      if (raw === null) return null
      return raw.startsWith('session-') ? raw : null
    })()
    /** 镜像实测的显示名（若镜像体可读且含该 id）。 */
    const mirrorDisplayTitle = (() => {
      if (mirrorBody.reachable !== true || mirrorBody.byId === undefined) return null
      for (const [id, entry] of Object.entries(mirrorBody.byId)) {
        if (id === acked.boundId || id === expectedIds[0]) return entry.displayTitle ?? null
      }
      return null
    })()
    /** 逐级取关键词：展示串优先，其次镜像 displayTitle 的实词，最后完整 id 兜底（三者都可命中 `displayTitle ?? id`）。 */
    const keywordPlan = [
      { source: 'host-approved-ui-id', kw: hostApprovedUiId },
      { source: 'mirror-displayTitle', kw: typeof mirrorDisplayTitle === 'string' && mirrorDisplayTitle.trim().length >= 2 ? mirrorDisplayTitle.trim() : null },
      { source: 'host-approved-id-fallback', kw: typeof acked.boundId === 'string' && acked.boundId.length > 0 ? acked.boundId : null },
    ]
    const effectiveKeyword = (keywordPlan.find((x) => typeof x.kw === 'string' && x.kw.length > 0) ?? { source: null, kw: null })

    await setInput('.nv-csearch input', effectiveKeyword.kw === null ? 'zzz-no-keyword-available' : effectiveKeyword.kw)
    await sleep(2400)
    const sessionRows = await cdp.evaluate(`(() => ({
      kw: (document.querySelector('.nv-csearch input') || {}).value,
      foundTitle: (document.querySelector('.nv-cfound-title') || {}).textContent ?? null,
      cfoundPresent: document.querySelector('.nv-cfound') !== null,
      rows: [...document.querySelectorAll('.nv-cfound .nv-srow')].map((n) => ({ t: (n.querySelector('.nv-srow-title') || {}).textContent, sub: (n.querySelector('.nv-srow-sub') || {}).textContent })),
      cards: document.querySelectorAll('.nv-cgrid .nv-ccard').length,
    }))()`)
    report.facts.sessionSearchAuthoritative = sessionRows
    // 镜像本体读数与会话行读数一并落证（判定依据 = **镜像是否反映宿主已认可的会话**，不以 innerText 启发式做 N-A 理由）。
    const hostRowsAtB21 = await readHostSessionRows(cdp)
    report.facts.hostSessionRowsAtB21 = hostRowsAtB21
    /** 行标题可核：每行标题非空（**不**要求标题含关键词——`displayTitleOf` 可能返回「目录名/id 之外的串」（如 cwd 名），见 README 对该口径的登记）。 */
    const rowTitles = sessionRows.rows.map((r) => r.t ?? null)
    const titleOk = sessionRows.rows.length >= 1 && sessionRows.rows.every((r) => typeof r.t === 'string' && r.t.length > 0)
    const b21Measurement = {
      ackComplete, kwOk: effectiveKeyword.kw !== null, effectiveKeyword: effectiveKeyword.kw, keywordSource: effectiveKeyword.source,
      ids: mirrorBody.reachable === true ? mirrorBody.ids : null, displayTitle: mirrorDisplayTitle,
      rows: sessionRows.rows.length, rowTitles, titleOk,
      mirrorBodyReachable: mirrorBody.reachable === true,
    }
    report.facts.b21Measurement = b21Measurement
    const b21aOk = evalPred('B21a-search-find-sessions', b21Measurement)
    assertion('B21a-search-find-sessions', '控制台/搜索', '关键词（取自宿主已认可会话的 UI 展示串 / 镜像 displayTitle，必然命中 `displayTitle ?? id`）→ 「找到的会话」MUST 渲染 ≥1 行且行标题可核；镜像本体为空或 0 行 ⇒ FAIL（R2 N-01 前提重设计；谓词取自共享登记表 `B21a-search-find-sessions`）',
      b21aOk,
      { ...b21Measurement, sessionRows, hostRowsAtB21, expectedIds, mirrorBody, keywordPlan, predicateId: 'B21a-search-find-sessions' },
      ackComplete === true ? undefined : 'N-A')
    // 点会话行 → 打开该会话并关控制台（仅在有行时可判；无行属前置未取得，与 D-3 判定无关）
    if (sessionRows.rows.length >= 1) {
      ...
      assertion('B22-session-row-opens', '控制台/搜索', '点「找到的会话」行 → 打开会话并关控制台（前置：镜像有会话行；本次检索 0 行 ⇒ 无可点行，如实记 N-A，不据此判缺陷）',
        false, { sessionRows, b21Measurement, reason: '本次检索 0 行（关键词 = ' + String(effectiveKeyword.kw) + '）⇒ 无「找到的会话」行可点' }, 'N-A')
    }
    ...
    const mirrorProbe = await cdp.evaluate(`(async () => {
      ...
      // R2 N-01/N-03：关键词由**调用方**（Node 侧）按「宿主已认可会话的实际展示串」注入，不再用无交集固定串。
      set(${JSON.stringify(effectiveKeyword.kw === null ? 'zzz-no-keyword-available' : effectiveKeyword.kw)})
      await sleep(2600)
      const out = {
        consoleOpen: true, attempts, kw: inp.value,
        mirrorBody: (() => {
          try {
            const realms = [document]
            try { for (const f of document.querySelectorAll('iframe')) { try { if (f.contentDocument !== null) realms.push(f.contentDocument) } catch (e) { /* cross-origin */ } } } catch (e) { /* ignore */ }
            ...
            for (const doc of realms) {
              ...
              for (const n of nodes) {
                ...
                for (const k of keys) {
                  let v = null
                  try { v = n[k] } catch (e) { continue }
                  if (v !== null && v !== undefined && typeof v === 'object' && Array.isArray(v.ids) && v.byId !== null && v.byId !== undefined) found.push(v)
                }
              }
            }
            if (found.length === 0) return { reachable: false, reason: 'no-mirror-node-found', realms: realms.length, scanned }
            const b = found[0]
            return { reachable: true, realms: realms.length, scanned, ids: b.ids.slice(0, 20), byId: Object.fromEntries(b.ids.slice(0, 20).map((id) => [id, { displayTitle: (b.byId[id] ?? {}).displayTitle ?? null }])) }
          } catch (e) { return { reachable: false, reason: 'exception: ' + String(e).slice(0, 200) } }
        })(),
    ...
    const b21bMirrorBody = mirrorProbe.mirrorBody ?? { reachable: false, reason: 'not-collected' }
    const b21bMirrorIds = b21bMirrorBody.reachable === true ? b21bMirrorBody.ids : null
    const b21bDisplayTitle = (() => {
      if (b21bMirrorBody.reachable !== true) return null
      for (const [id, entry] of Object.entries(b21bMirrorBody.byId ?? {})) if (id === acked.boundId) return entry.displayTitle ?? null
      return null
    })()
    const b21bRowTitles = Array.isArray(mirrorProbe.rows) ? mirrorProbe.rows.map((t) => t ?? null) : []
    const b21bMeasurement = {
      ackComplete, consoleOpen: mirrorProbe.consoleOpen === true,
      kwOk: effectiveKeyword.kw !== null, effectiveKeyword: effectiveKeyword.kw, keywordSource: effectiveKeyword.source,
      ids: b21bMirrorIds, displayTitle: b21bDisplayTitle,
      rows: b21bRowTitles.length, rowTitles: b21bRowTitles,
      titleOk: b21bRowTitles.length >= 1 && b21bRowTitles.every((t) => typeof t === 'string' && t.length > 0),
      mirrorBodyReachable: b21bMirrorBody.reachable === true,
    }
    report.facts.b21bMeasurement = b21bMeasurement
    const b21bOk = evalPred('B21b-search-find-sessions-retest', b21bMeasurement)
    if (mirrorProbe.consoleOpen !== true) {
      ...
      assertion('B21b-search-find-sessions-retest', '控制台/搜索', '收尾复测（控制台已在场）：以「宿主已认可会话的实际展示串」为关键词检索 ⇒ MUST 渲染 ≥1 行且行标题可核；镜像本体为空或 0 行 ⇒ FAIL（R2 N-01 前提重设计；谓词取自共享登记表 `B21b-search-find-sessions-retest`）',
        b21bOk,
        { ...b21bMeasurement, mirrorProbe, predicateId: 'B21b-search-find-sessions-retest' },
        ackComplete === true ? undefined : 'N-A')
    }
    ...
    // R2 N-02 修复：自证方**消费共享登记表**，并把运行期部署消费点（`evalPred` 计数 + 谓词源 sha256）
    // 一并落证 ⇒ 「部署面 = 自证面」可机读核对；非同源（登记谓词无部署消费点）会同时体现在
    // `deployments[].consumed=false` 与 `falsifiability-check.mjs` 的绑定检查上。
    report.falsifiability = falsifiabilityReport(predicateTake, staticCallSiteCounts())
    report.predicateDeployments = Object.fromEntries(Object.entries(predicateTake).map(([k, v]) => [k, { count: v.count, sha: v.sha }]))
    report.predicateStaticSites = Object.fromEntries(report.falsifiability.rows.map((r) => [r.id, r.deployment.staticCallSites]))
    assertion('FALSIFIABILITY-PROOF', '测试资产', '新/改谓词的「构造反例即红」自证：共享登记表（`PREDICATE_REGISTRY`）逐条谓词 MUST 对 ≥1 反例向量求值为 false、对 ≥1 正例向量求值为 true（无恒真分支），且 MUST 在源码面有 ≥1 处 `evalPred` 部署消费点（同源）；运行期被走到的谓词其函数源指纹 MUST 与自证面逐字一致',
      report.falsifiability.allPass === true
      && report.falsifiability.rows.every((r) => r.deployment.staticBound === true)
      && report.falsifiability.rows.every((r) => r.deployment.consumed !== true || r.fnSha256 === r.deployment.fnSha256),
      {
        vectorTally: report.falsifiability.vectorTally,
        staticSites: report.predicateStaticSites,
        runtimeSites: report.predicateDeployments,
        summary: report.falsifiability.rows.map((r) => ({ id: r.id, finding: r.finding, fnSha256: r.fnSha256.slice(0, 12), staticBound: r.deployment.staticBound, staticCallSites: r.deployment.staticCallSites, runtimeCallSites: r.deployment.runtimeCallSites, redCount: r.redCount, okCount: r.okCount, redsAllFalse: r.redsAllFalse, oksAllTrue: r.oksAllTrue, pass: r.pass })),
        detail: report.falsifiability.rows,
      })
