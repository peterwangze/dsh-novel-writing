...
export const hostContract = {
  ...
  items: [
    ...
    { face: 2, item: '2.12', kind: 'event', symbol: "ctx.on('internal/service')（单一监听器双消费：launcher 重 setup + hook.refresh）", file: 'lib/client.js', line: 'L4403-4408', necessity: 'required', incident: 'BUG-004+BUG-005', note: 'cordis 4.x 事件（EVD-077 实证）；服务缺席↔到位自愈' },
    ...
    { face: 3, item: '3.5', kind: 'dom-interop', symbol: ".ta_splitClose 类名点击 + 'dsh:split-claim' CustomEvent（与 dsh-worktable 跨插件分栏互操作）", file: 'lib/client.js', line: 'L896-907', necessity: 'optional', note: '生态约定非宿主官方；try/catch 包裹 + claim 监听让位（L4406-4417）' },
    ...
    // ── 面 6 — 版本与环境面（5 项）──
    { face: 6, item: '6.1', kind: 'version-policy', symbol: 'peerDependencies 全 *（cordis / schemastery / dsh-settings / dsh-home-paths / dsh-tools）', file: 'package.json', line: 'L38-44', necessity: 'improvable', note: '* = 不设限=不设防（无下限=隐式信任任意未来版本）；junction 共享 profile node_modules 保证闭包内一致；CI 依赖架构守卫（ci.yml L24-37）；四次事故都在 * 口径下发生' },
