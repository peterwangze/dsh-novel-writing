...
export const hostContract = {
  ...
  revisions: [
    ...
    { task: 'CLEAN-006', scope: '**面 2/3 行号重基（F1：会话镜像 hook 改走 `useSyncExternalStore`——BUG-009 R1 / RISK-008）**。`lib/client.js` **5233 → 5268 行**（净 +35）。**受影响引用点 11 处（其中 `lib/client.js` 面 9 处；另 2 处属 `lib/index.js`）**。**插入点（坐标基准：`new L<n>` = 插入行在新文件中的行号；`old L<n> 前` = 该插入发生在旧文件第 n 行之前）**：① **替换** `new L100`（`const { useState, useEffect, useRef, useMemo } = react` → 追加 `useSyncExternalStore`，Δ0——同址替换，**不属「重基项」**）；② 插入 `new L1451-L1452`（`old L1467` 前，**+2**：工厂级 `ABSENT_SNAP` 单例——`getSnapshot` MUST NOT 每渲染新建对象）；③ 插入 `new L1469-L1500`（`old L1506` 前，**+32**：`getSnapshotForRender` + `subscribeForCurrent` 稳定包装 + 注释块）；④ 插入 `new L1540`（`old L1540` 前，**+1**：`useSyncExternalStore(...)` 调用点）。**Δ 分段（old 坐标 → Δ，作用于「同一文件内」的引用）**：`L1-1450: 0 ｜ L1451-1466: +2 ｜ L1467-1505: +34 ｜ L1506-end: +35`。\n**重基口径（R1 **F-01** 订正）**：`line` 字段的每个数值 MUST **按其所属 `file` 分组**计算 Δ——**跨文件引用不得套用本文件的 Δ**（1.6/1.9 的调用点段指向 `lib/index.js`，该文件本批零改动 ⇒ 其真值不变；R1 发现的误加 +2 已回滚）。\n**变更条目：10 项 / 受影响引用点 11 处**（`2.2` 的 `line` 值 `L97` 未变、不属重基项）：`2.10`（`L1449-1513`→`L1449-1548`）、`2.11`（`L1541-1568`→`L1576-1603`）、`2.12`（`L5125-5132`→`L5160-5167`）、`2.13`（`L5162-5194`→`L5197-5229`）、`2.3`（`L5095-5097`→`L5130-5132`）、`3.4`（`L925-938 / L4746 / L4889`→`L925-938 / L4781 / L4924`）、`3.6`（`L1164-1188 / L3886`→`L1164-1188 / L3921`）、`3.8`（`L2983-2993`→`L3018-3028`）、`1.6`/`1.9`（**跨文件误重基后回滚**，见下）。**1.6/1.9 本批净效果 = `line` 值不变**（`L55（调用点 L1457、L95）` / `L76 / L1463`）。\n**双取证（逐项非估计值）**：① **逐行等价**——`lib/client.js` 面 9 处 32 段全部满足 `old[L−Δ(L)] ≡ new[L]`（首行 + 范围末行逐字节比对，**32/32 等价、0 处不等价**）；**跨文件引用另按其目标文件行号核验**（`lib/index.js:1457` = `registerSettings(ctx, NS, Config)`、`:1463` = `onLifecycle(ctx, …)`、`:95` = `const c = readSettings(ctx, NS)`，逐行实读）；② **插入点独立重算**——LCS 差分（`difflib.SequenceMatcher`，`git diff --no-index` 交叉印证）求得**恰好 4 个 opcode**（1 替换 + 3 插入），逐点 Δ 与上式一致、总 Δ `+35` ≡ 文件行数差。\n**未变**：items[] 计数 49 与 face 分布、kind/necessity/symbol/`golden` 面计数、`regionLiterals`、`presetRowConfig[]` 与 item 5.3/5.4/5.6（BUG-007 交付面）**零改动**。伴随订正：`test/smoke.mjs`/`README.md` 内 `<item> L<n>` 陈旧副本随重基同步（否则 COMPAT-015 F3 行号引用对账转红）。' },
    { task: 'CLEAN-006（R1 返工）', scope: '**F1 通道回调转发（R1 **F-04**）+ 面 2/3 增量重基**：`subscribeForCurrent(onStoreChange)` 改为**原样转交 React 回调**（原实现另订阅 `noop`、丢弃回调 ⇒ React 变更通知链永不触发；注释与 CHANGELOG 的机制陈述同步订正为真实分工）。`lib/client.js` **5268 → 5273 行**（净 +5，3 个替换段：`new L1480-1489` / `L1495-1497` / `L1502-1503`，**全部落在 `old L1480-1498` ⇒ 对 old 坐标 `≤1498: 0 ｜ ≥1499: +5`**）。**重基 8 项 / 9 处（全部 `lib/client.js`）**：`2.10`（`L1449-1548`→`L1449-1553`）、`2.11`（`L1576-1603`→`L1581-1608`）、`2.12`（`L5160-5167`→`L5165-5172`）、`2.13`（`L5197-5229`→`L5202-5234`）、`2.3`（`L5130-5132`→`L5135-5137`）、`3.4`（`L925-938 / L4781 / L4924`→`L925-938 / L4786 / L4929`）、`3.6`（`L1164-1188 / L3921`→`L1164-1188 / L3926`）、`3.8`（`L3018-3028`→`L3023-3033`）。**取证口径（按 file 分组 + 双取证）**：① 逐行等价 `old[L−5] ≡ new[L]`（8 项 / 9 处 32 段逐字节，**全等价 0 处不等价**）；② Δ 分段由 LCS 差分独立重算（3 opcode、净 +5 ≡ 行数差）。**未变**：items[] 49 与 face 分布、非 `line` 字段零变更、`regionLiterals`/`presetRowConfig[]`/`kindEnum`/`faces` 逐字节相同、item 5.3/5.4/5.6 未动。' },
    { task: 'CLEAN-006（R2 返工）', scope: '**F-04 陈述订正引发的面 2/3 增量重基（局部 +2）**：`subscribeForCurrent` 的机制注释按 R2 **N-4** 改写（7 行 → 11 行 ⇒ 净 +4）并经 R2 N-1 重构后再调整（本批净 **+2**）。`lib/client.js` **5273 → 5275 行**。**Δ 分段（old = `2bf58d2` 坐标）**：`≤1478: 0 ｜ 1479-1482: **改写区**（4 行 → 6 行；该区为注释块，无锚点条目落于其中 ⇒ 区间内不谈 Δ）｜ ≥1483: **+2**。**重基 8 项 / 11 段（全部 `lib/client.js`；按目标 file 分组）**：`2.10`（`L1449-L1553`→`L1449-L1555`——**跨改写区，起止 Δ 不同**：起点 1449 < 1479 ⇒ Δ0，终点 1553 > 1482 ⇒ Δ+2；此类条目 MUST 逐段各自算 Δ，不得整串套用）、`2.11`（`L1581-L1608`→`L1583-L1610`）、`2.12`（`L5165-L5172`→`L5167-L5174`）、`2.13`（`L5202-L5234`→`L5204-L5236`）、`2.3`（`L5135-5137`→`L5137-L5139`）、`3.4`（`L925-938 / L4786 / L4929`→`L925-938 / L4788 / L4931`；首段在改写区前 ⇒ Δ0）、`3.6`（`L1164-1188 / L3926`→`L1164-1188 / L3928`）、`3.8`（`L3023-L3033`→`L3025-L3035`）。**格式一并归一**：`2.10` 原 `L1449-L1553` 属本仓唯一「双 L」写法（R1 重基时引入），本次改为与其余条目同形的 `L1449-1555`（R2 **N-6** 精度要求；`COMPAT-012 N2 F5d` 的范围正则接受两种写法，但同形可读性更佳）。**取证口径（双取证）**：① **改写区外逐行等价** `old[L] ≡ new[L+Δ(L)]`（5269 行比对、**neq=0**）；② **改写区独立重算**（最长公共前缀 1478 行 + 最长公共后缀 3791 行 ⇒ 单区间 4 行 → 6 行，逐点 Δ 与 Δ 分段一致、局部 +2 ≡ 本批新增行数差）。**未变**：items[] 计数 49 与 face 分布、`kind`/`necessity`/`symbol`/`golden` 面计数、`regionLiterals`、`presetRowConfig[]`、`kindEnum`/`faces`、item 5.3/5.4/5.6 零改动；`revisions[]` 10 → 11。伴随订正：`test/smoke.mjs` 注释内 `<item> L<n>` 陈旧副本随本次重基同步（否则 COMPAT-015 F3 行号引用对账转红），并修正 `2.10` 终点在本批前即已偏移 2 行的历史残差（终点由函数内 `refresh` 收尾 `}` 校正为**函数收尾 `    }`**）。' },
  ],
  faces: [
  ...
  items: [
    ...
    { face: 1, item: '1.9', kind: 'api-call', symbol: 'ctx.effect(fn, label)（生命周期清理，cordis 核心；边界 onLifecycle）', file: ['lib/host-boundary.js', 'lib/index.js'], line: 'L76 / L1463', necessity: 'required', note: 'COMPAT-004 收口（原 lib/index.js L1429 为收口前历史值 → 边界 onLifecycle；index.js 调用点行号见本项 `line` 字段 = apply 内 onLifecycle 调用）——**第二段位于 `lib/index.js`**，其行号只随 **index.js 自身**的 Δ 变化（CLEAN-006 R1 **F-01** 订正：原按 client.js 的 Δ 误加 +2 已回滚为真值）' },
    ...
    { face: 3, item: '3.6', kind: 'dom-observer', symbol: "ResizeObserver + MutationObserver(attributeFilter:['data-phase'])（DOM/尺寸变化跟随重锚定）", file: 'lib/client.js', line: 'L1164-1188 / L3928', necessity: 'required', incident: 'RISK-003', note: '探测的是变化后的自救，不是宿主约定的变更探测。行号重基（COMPAT-005）。**note 口径（REVIEW-COMPAT-005-R1 F5）**：不内嵌「现 L…」行号（行号以 `line` 字段为唯一事实源——原内嵌副本为字段旧值，残留即成 stale）' },
    ...
    { face: 3, item: '3.8', kind: 'dom-selector', symbol: "findSidebarEl() 侧栏探测：抽屉挂载点向上找 aside/nav 标签 + 宿主类名正则 /SidebarRoot|sidebar/i + document.querySelector('aside, nav, [class*=\"SidebarRoot\"], [class*=\"sidebar\"]') 兜底", file: 'lib/client.js', line: 'L3025-L3035', necessity: 'required', incident: 'RISK-003', note: 'REVIEW-COMPAT-002-R1 F1 补录（§3 底稿未列、面 3 分布 7→8）：宿主侧栏类名/结构变更时契约面原为零信号；dsh-worktable 同款 findSidebar 约定；返回 null → 几何回退 left=280 默认值且不监听侧栏尺寸（降级无白屏）。行号重基（COMPAT-005；行号以本项 `line` 字段为唯一事实源，note 不内嵌副本）' },
