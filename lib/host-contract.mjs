...
export const hostContract = {
  ...
  items: [
    ...
    { face: 3, item: '3.1', kind: 'dom-selector', symbol: "document.querySelectorAll('[data-phase]')（findConversationRoot：phase=active 优先 + children≥2 + 排除 TEXTAREA/INPUT）", file: 'lib/client.js', line: 'L987-995', necessity: 'required', incident: 'RISK-003', note: '宿主无官方布局 API；dsh-worktable 同款约定；null → 不做布局降级（实读同函数内 null 分支）。行号重基（COMPAT-005）。**note 口径（REVIEW-COMPAT-005-R1 F5）**：不内嵌「现 L…」行号（行号以 `line` 字段为唯一事实源）；起于 JSDoc `/**` ——非严格面合法注释起段，由 smoke ⑫d golden 锁定。**行号重基（UX-060）**：告警流式面新增段落在本项之前 ⇒ 机械偏移 +9，起点仍为 JSDoc `/**` 注释行（非严格面合法注释起段计数 2 不变）（逐行实读复核）' },
    ...
    { face: 3, item: '3.5', kind: 'dom-interop', symbol: ".ta_splitClose 类名点击 + 'dsh:split-claim' CustomEvent（与 dsh-worktable 跨插件分栏互操作）", file: 'lib/client.js', line: 'L1037-1049', necessity: 'optional', note: '生态约定非宿主官方；try/catch 包裹 + claim 监听让位（让位段 = `apply` 内 `claimHandler` 段，与 2.12 的 `offServiceEvent` 同区）。**行号重基（COMPAT-005；行号以本项 `line` 字段为唯一事实源，note 不内嵌副本）**；起于互操作说明前的空行段——非严格面合法起段，由 smoke ⑫d golden 锁定（恰 2 处之一，另一处 = 3.1 的 JSDoc）。**行号重基（UX-060）**：告警流式面新增段落在本项之前 ⇒ 机械偏移 +9，起点仍为空行段、终点仍为 `.ta_splitClose` 点击行（逐行实读复核）' },
