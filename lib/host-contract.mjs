...
export const hostContract = {
  ...
  items: [
    ...
    { face: 1, item: '1.2', kind: 'import', symbol: '@deepseek-ai/schemastery 默认导出 z（Config schema，settings.register 消费）', file: 'lib/host-boundary.js', line: 'L17', necessity: 'required', note: 'COMPAT-004 收口（原 lib/index.js L16）：index.js 经边界 import 后用于 Config 定义（L40 起）；CI 用真包（ci.yml L78，registry 安装——RISK-001：mock 无法覆盖链式 API）' },
    { face: 1, item: '1.3', kind: 'import', symbol: '@deepseek-ai/dsh-home-paths 具名导出 resolveDshHome（预设同步目标根路径解析，边界 presetRoot/presetDir 包装）', file: 'lib/host-boundary.js', line: 'L19（调用点 L98、L103）', necessity: 'required', note: 'COMPAT-004 收口（原 lib/index.js L18，调用点 L163/L1360 → 边界 presetRoot()/presetDir()，index.js L169/L1366）；与宿主 DSH_HOME 解析逻辑必须一致；CI mock（ci.yml L104-109）' },
    { face: 1, item: '1.4', kind: 'import', symbol: 'settingsNamespace 具名导入（已退役：BUG-003 修复改为字符串常量 L39）', file: 'lib/index.js', line: '原 L18 → 现 L39 字符串常量', necessity: 'eliminated', incident: 'BUG-003', note: '已消除但保留在册供历史勾稽（F10 口径）；CI mock 漂移见 6.4；COMPAT-004 后 NS 常量仍在 index.js（非宿主 API，普通字符串）' },
    { face: 1, item: '1.5', kind: 'inject', symbol: "inject = ['settings']（settings 硬依赖声明，cordis inject 机制；声明值来自边界 hostInject）", file: ['lib/index.js', 'lib/host-boundary.js'], line: 'L36 / L51', necessity: 'required', note: 'COMPAT-004 收口：字面量上移至边界 hostInject（L51），index.js 仅 re-export（L36）——声明值单一事实源' },
    { face: 1, item: '1.6', kind: 'api-call', symbol: 'ctx.settings.register(NS, Config) / ctx.settings.get(NS)（边界 registerSettings / readSettings）', file: 'lib/host-boundary.js', line: 'L55（调用点 L1429、L94）', necessity: 'required', incident: 'BUG-003', note: 'COMPAT-004 收口（原 lib/index.js L1423 / L88）：NS 形态 symbol→string 事故点；smoke 全量行为测试' },
    ...
    { face: 1, item: '1.8', kind: 'api-call', symbol: "ctx.get('webServer')（可选，边界 getWebServer）+ webServer.register({kind:'exact', path, handler}) ×13 路由（/novel-writing/api/*，边界 registerRoute）", file: 'lib/host-boundary.js', line: 'L66 / L71', necessity: 'required', note: 'COMPAT-004 收口（原 lib/index.js L1427-1431 / L1229-1289 → 边界 L66/L71，index.js 路由注册 L1236-1368（COMPAT-005 后 D1 段 → L1368-1400，含 compat 路由））。**路由数复核**：§3 底稿称 13；COMPAT-012 实测为 **12**（overview/novel/chapter/gate/request/novel-create/novel-delete/file/request-done/publish/data/preset-sync）——**COMPAT-005 D1 新增 1 条只读诊断路由 `compat` → 现为 13**，与底稿计数巧合一致（底稿未区分本条，故数值自洽但来源不同；路由数机检随契约本字段）。客户端轮询数据面；webServer 缺席已有降级警告（index.js L1437）+ [nv-compat] 告警项 1.8' },
    { face: 1, item: '1.9', kind: 'api-call', symbol: 'ctx.effect(fn, label)（生命周期清理，cordis 核心；边界 onLifecycle）', file: ['lib/host-boundary.js', 'lib/index.js'], line: 'L76 / L1435', necessity: 'required', note: 'COMPAT-004 收口（原 lib/index.js L1429 → 边界 onLifecycle，index.js L1435 调用）' },
    { face: 1, item: '1.10', kind: 'api-call', symbol: "ctx.emit('novel-writing/changed')（边界 emitChanged/EVENT_CHANGED）+ ctx.logger（边界 loggerOf/logWarn）", file: ['lib/host-boundary.js', 'lib/index.js'], line: 'L92 / L154 / L121', necessity: 'required', note: 'COMPAT-004 收口（原 lib/index.js L148 / L114）：事件名常量 EVENT_CHANGED 与 logger 取值均在边界，index.js 三个调用点（emit L154 / logger L121 / warn L1437）' },
    ...
    { face: 2, item: '2.3', kind: 'inject', symbol: "inject = ['slots']（硬依赖仅 UI 挂载通道）+ connection/locale 转**可选探测**（apply 期 ctx.get 惰性判定，缺席降级不崩溃）", file: 'lib/client.js', line: 'L4734-L4736', necessity: 'consolidatable', incident: 'BUG-004', note: '行号重基（COMPAT-005：原 L4374-L4377 → 现 L4723-L4725）；起点取**声明行本体**（A3 说明注释块 L4716-4722 为前导，面 2 严格面要求边界为代码行）。COMPAT-004 A3 已执行（DEC-026）：原三服务硬声明（旧 L4367）使插件在无 connection/locale 的宿主上不挂载——现收敛为 slots 硬依赖 + ctx.get 惰性探测（宿主 cordis get() 对未提供服务返回 undefined，非抛错）；legacyApi 回退分支零改动（见 2.4）；残余收敛方向 = 轴① A1 版本矩阵（COMPAT-006）' },
    { face: 2, item: '2.4', kind: 'api-call', symbol: 'connection.api（旧宿主 ≤0.1.x API 回退，全文件唯一特性检测点）', file: 'lib/client.js', line: 'L627 / L655 / L671 / L680 / L719 / L728', necessity: 'adapted', incident: 'BUG-004', note: 'makeHostApi 单点收口；代码级引用恰 1 处（EVD-077 审查复核 + smoke 断言）。行号重基（COMPAT-005：原 L549 / L573 / L589 / L598 / L637 / L646 → 六处 legacyApi 回退返回逐点实读）' },
    { face: 2, item: '2.5', kind: 'remote-namespace', symbol: 'remote.settings（describe/update/mutate；位置参数 + expectedRevision acceptsUndefined 形状）', file: 'lib/client.js', line: 'L646-656', necessity: 'required', incident: 'BUG-004', note: '12 组形状映射之一（makeHostApi）。行号重基（COMPAT-005：原 L564-574 → L634-644；终点取闭合 `},` 保持面内无缝隙）' },
    { face: 2, item: '2.6', kind: 'remote-namespace', symbol: 'remote.session（create/prompt/cancel；requestId 必填铸造 L576-584）', file: 'lib/client.js', line: 'L657-665', necessity: 'required', incident: 'BUG-004', note: '行号重基（COMPAT-005：原 L575-590 → L645-653；起点由 BUG-004 注释块内行修正为 getter 行本体）' },
    { face: 2, item: '2.7', kind: 'remote-namespace', symbol: 'remote.workspace + workspaces 快照服务（list=getSnapshot/subscribe、phase pending→ready、awaitWorkspacesReady 订阅等待 L586-612）', file: 'lib/client.js', line: 'L673-700', necessity: 'required', incident: 'BUG-004', note: '行号重基（COMPAT-005：原 L591-618 → L661-687；终点取 workspace getter 闭合行，不侵入 2.8 的 getter）' },
    { face: 2, item: '2.8', kind: 'remote-namespace', symbol: 'remote.directoryPicker（pick string→{path} 包装 L697 / createDirectory / list）', file: 'lib/client.js', line: 'L701-720', necessity: 'required', incident: 'BUG-004', note: '行号重基（COMPAT-005：原 L619-638 → L689-708）' },
    { face: 2, item: '2.9', kind: 'remote-namespace', symbol: 'remote.agentPresets（select 两参位置）', file: 'lib/client.js', line: 'L721-730', necessity: 'required', incident: 'BUG-004', note: '行号重基（COMPAT-005：原 L639-647 → L709-718；起点由前项注释尾行修正为 getter 行本体）' },
    { face: 2, item: '2.10', kind: 'reactive-store', symbol: 'sessions.list ObservableSnapshot（getSnapshot/subscribe 形状校验 L1288-1291，makeSessionsHookReactive）', file: 'lib/client.js', line: 'L1294-1358', necessity: 'required', incident: 'BUG-005', note: '会话状态镜像（零轮询）。行号重基（COMPAT-005：原 L1212-1276 → L1289-1353，整函数跨度不变；形状校验实读 L1295-1298）' },
    { face: 2, item: '2.11', kind: 'api-call', symbol: 'launcher.sessions（sessions.open(sessionId) 打开/切换会话）', file: 'lib/client.js', line: 'L1382-1409', necessity: 'required', note: '行号重基（COMPAT-005：原 L1317-1327 → L1377-1404）；起点回到控制器对象声明行 `const launcher = {`（原值起点落在无关行，⑫b 判据①③自 004-R1 起即报偏移）——终点取 open() 闭合行，⑤b 构造闭合落在范围内' },
    { face: 2, item: '2.12', kind: 'event', symbol: "ctx.on('internal/service')（单一监听器三消费：locale 重快照 + launcher 重 setup + hook.refresh）", file: 'lib/client.js', line: 'L4763-4770', necessity: 'required', incident: 'BUG-004+BUG-005', note: 'cordis 4.x 事件（EVD-077 实证）；服务缺席↔到位自愈。**描述订正（COMPAT-013 F-4，只读 client.js 逐行实读）**：原记「单一监听器**双消费**（launcher 重 setup + hook.refresh）」已 stale——REVIEW-COMPAT-004-R1 F2 为 locale「后到」自愈在同一监听器内加了 `locale` 分支（`snapshotLocale(ctx)` + `rerender()`，实读 L4768），实为 **3 消费者**（L4768 locale / L4770 launcher / L4771 hook.refresh）。**行号重基（COMPAT-005：原 L4403-4408 → L4763-4770）**；起点取声明行 `const offServiceEvent = …` 本体（旧值起点落在无关 UX-026 注释行上，⑫b 判据①③自 004-R1 起即报偏移）' },
    { face: 2, item: '2.13', kind: 'slot', symbol: "ctx.slots.inject('settings.section'|'sidebar.footer.action'|'shell.overlay') + ctx.slots.register({name,id,order,label}) ×6 注册", file: 'lib/client.js', line: 'L4790-4822', necessity: 'required', note: 'UI 挂载槽位名约定，无替代挂载通道；槽位名稳定性 TP-2 待验证。**行号重基（COMPAT-005：原 L4429-4461 → L4793-4825）**：6 处 `ctx.slots.inject(...)` 与 6 处 `ctx.slots.register(...)` 全部落在范围内（judge ⑤a 计数 6 ≡ 6），终点取末次 register 的 `))` 闭合行。**N3 行为边界（REVIEW-COMPAT-004-R2，COMPAT-005 登记）**：宿主侧槽位**标签**（本项 label()）的刷新依赖「宿主在 locale 变化后重渲染自身 shell」——本侧已保证**必要条件**（label 为渲染期惰性取值，smoke ④b 实测 zh⇒en），但「宿主会重渲染」这一前提**在本仓无证据（前提未证实）**：若宿主不重渲染 shell，宿主侧标签滞后到下一次自然重渲染（自有 React 组件经 store 空通知已即时刷新）。**N4 已知边界（同轮，COMPAT-005 登记）**：locale 服务**撤离**时保持最后一次快照（不强行回退 zh——避免宿主热重载期间标签抖动），撤离同样触发一次空重渲染（无害）' },

    // ── 面 3 — 宿主 DOM/前端约定面（8 项）──
    // 行号重基（COMPAT-005 D1，同面 2）：均 +350（面板插入点位于本段之前）。
    { face: 3, item: '3.1', kind: 'dom-selector', symbol: "document.querySelectorAll('[data-phase]')（findConversationRoot：phase=active 优先 + children≥2 + 排除 TEXTAREA/INPUT）", file: 'lib/client.js', line: 'L920-928', necessity: 'required', incident: 'RISK-003', note: '宿主无官方布局 API；dsh-worktable 同款约定；null → 不做布局降级（实读 L978-979）。行号重基（COMPAT-005：原 L838-846 → L908-916；起于 JSDoc `/**` ——非严格面合法注释起段，由 smoke ⑫d golden 锁定）' },
    { face: 3, item: '3.2', kind: 'dom-structure', symbol: 'root.children[0]=header / root.children[1]=viewArea 结构假设', file: 'lib/client.js', line: 'L992-993 / L1076', necessity: 'required', incident: 'RISK-003', note: 'undefined 检查降级（L982 return false）。行号重基（COMPAT-005：原 L910-911 / L994 → L980-981 / L1064）' },
    { face: 3, item: '3.3', kind: 'layout-technique', symbol: 'margin 挤压（marginLeft/Right/Top + width 双压——UX-018 修复：margin 不触发宿主响应式重排，需显式 width）', file: 'lib/client.js', line: 'L1004-1007 / L1084-1089 / L1141-1143', necessity: 'required', incident: 'RISK-003 + UX-018', note: 'dsh-worktable 同款；saved* 原值恢复机制完备 + 让位观察器（L1030-1042）。行号重基（COMPAT-005：原 L922-925 / L1003-1014 / L1060-1062 → L992-995 / L1073-1084 / L1130-1132）' },
    { face: 3, item: '3.4', kind: 'css-token', symbol: '宿主设计令牌：--dsw-alias-* 族（TK 令牌表 + NV_STYLE 全量 var(--dsw-alias-*, 兜底值) + color-mix 派生 L4687-4693）+ --dsw-shadow-lv2（弹窗阴影，带兜底）', file: 'lib/client.js', line: 'L779-792 / L4685 / L4835', necessity: 'required', note: '全文件去重令牌名恰 13 个 = alias 族 12 + shadow 族 1（REVIEW-COMPAT-002-R1 F2 修订：族外盲区关闭，见 regionLiterals.cssTokens）；TK 表 L767-780 仅含 11 个去重 alias 名，第 12 个 --dsw-alias-bg-elevated 出自 L4821，--dsw-shadow-lv2 出自 L4686/L4821；兜底取值对齐 dsh-worktable styles.ts；令牌稳定化 TP-3 待验证。**行号重基（COMPAT-005：原 L697-710 / L4136 / L4271 → L767-780 / L4686 / L4821；新增 D1 面板样式（L4679-4693）未引入族外令牌**）' },
    { face: 3, item: '3.5', kind: 'dom-interop', symbol: ".ta_splitClose 类名点击 + 'dsh:split-claim' CustomEvent（与 dsh-worktable 跨插件分栏互操作）", file: 'lib/client.js', line: 'L970-982', necessity: 'optional', note: '生态约定非宿主官方；try/catch 包裹 + claim 监听让位（**L4757-4770**）。**行号重基（COMPAT-005：原 L896-907 → L970-981；claim 让位段原 L4411-4424 → L4757-4770）**；起于 `//` 互操作说明——非严格面合法注释起段，由 smoke ⑫d golden 锁定（恰 2 处之一，另一处 = 3.1 的 JSDoc；起点取 `//` 说明注释行）' },
    { face: 3, item: '3.6', kind: 'dom-observer', symbol: "ResizeObserver + MutationObserver(attributeFilter:['data-phase'])（DOM/尺寸变化跟随重锚定）", file: 'lib/client.js', line: 'L1018-1042 / L3635', necessity: 'required', incident: 'RISK-003', note: '探测的是变化后的自救，不是宿主约定的变更探测。行号重基（COMPAT-005：原 L934-957 / L2793 → L1004-1027 / L3360）' },
    { face: 3, item: '3.7', kind: 'storage-key', symbol: "localStorage 键：dsh.novel.split.v1 / dsh.novel.defaults.v1 / dsh.novel.order.v1", file: 'lib/client.js', line: 'L826-828 等', necessity: 'own', note: '自有口径统一=1 项（REVIEW-COMPAT-001-R1 F10）：插件自有点、无宿主耦合，列出为完整性。行号重基（COMPAT-005：原 L744-746 等 → L814-816 等；本项非范围形态，不入 ⑫b 范围条目计数）' },
    { face: 3, item: '3.8', kind: 'dom-selector', symbol: "findSidebarEl() 侧栏探测：抽屉挂载点向上找 aside/nav 标签 + 宿主类名正则 /SidebarRoot|sidebar/i + document.querySelector('aside, nav, [class*=\"SidebarRoot\"], [class*=\"sidebar\"]') 兜底", file: 'lib/client.js', line: 'L2757-2767', necessity: 'required', incident: 'RISK-003', note: 'REVIEW-COMPAT-002-R1 F1 补录（§3 底稿未列、面 3 分布 7→8）：宿主侧栏类名/结构变更时契约面原为零信号；dsh-worktable 同款 findSidebar 约定；返回 null → 几何回退 left=280 默认值且不监听侧栏尺寸（实读 L3107-3114 / L3122-3123，降级无白屏）。行号重基（COMPAT-005：原 L2411-2421 → L2746-2756）' },
  ...
  clientProbes: [
    ...
    { item: '3.6', probe: 'dom-observers', mode: 'runtime-dom', kind: 'dom-observer', note: 'ResizeObserver/MutationObserver 构造子在 bindow 环境在场性（DOM 变更自救能力的基础设施）' },
  ],
