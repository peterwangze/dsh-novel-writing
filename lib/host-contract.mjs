 ...
 *                     供 smoke「契约声明面 ⊆ fixture 实测面」与 ci.yml mock 面钉（F1）消费；fixtures 由
 *                     test/fixtures/host-surfaces/extract.mjs 从真实包只读提取（禁止手写表面）。
 */
...
export const hostContract = {
  ...
  items: [
    // ── 面 1 — 服务端 API 面（11 项）──
    { face: 1, item: '1.1', kind: 'import', symbol: "@deepseek-ai/cordis 具名导出 Service（服务基类，cordis 插件模型本体）", file: 'lib/index.js', line: 'L17', necessity: 'required', note: 'CI mock（ci.yml L79-82）+ BUG-003 后逐项核验（EVD-076）' },
    { face: 1, item: '1.2', kind: 'import', symbol: '@deepseek-ai/schemastery 默认导出 z（Config schema，settings.register 消费）', file: 'lib/index.js', line: 'L16', necessity: 'required', note: 'CI 用真包（ci.yml L78，registry 安装——RISK-001：mock 无法覆盖链式 API）' },
    { face: 1, item: '1.3', kind: 'import', symbol: '@deepseek-ai/dsh-home-paths 具名导出 resolveDshHome（预设同步目标根路径解析）', file: 'lib/index.js', line: 'L18（调用点 L163、L1360）', necessity: 'required', note: '与宿主 DSH_HOME 解析逻辑必须一致；CI mock（ci.yml L104-109）' },
    ...
    { face: 5, item: '5.5', kind: 'preset-location', symbol: '$DSH_HOME/.agent-presets/<preset-id>/ 预设目录约定（install 同步 + index.js ensurePreset 双端落地）', file: ['install.ps1', 'install.sh', 'lib/index.js'], line: 'L306-316 / L207-216 / L157-184', necessity: 'required', incident: 'BUG-006（环境连锁，见 4.5）', note: '版本标记幂等；F9 来源注记：本行段为 §3 原始分段（仅预设目录同步子面），4.5 因含 settings.yaml 默认节写入而合并为 L306-339 / L207-232——两者并列非重复计数' },
  ...
  hostSurface: {
    ...
    ciMock: {
      ...
      heredocTarget: 'node_modules/@deepseek-ai/dsh-settings/index.js',
      note: 'ci.yml 内 dsh-settings mock 的运行时导出键集 MUST 恰等于 requiredExports[本包]（REVIEW-COMPAT-002-R1 F1 面钉）——mock 多导出宿主已删除的表面 = BUG-003 真绿假绿温床（COMPAT-010 已修正该实例）',
    },
    versionFacts: [
