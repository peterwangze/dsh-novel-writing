...
export const hostContract = {
  ...
  items: [
    ...
    { face: 2, item: '2.3', kind: 'inject', symbol: "inject = ['slots']（硬依赖仅 UI 挂载通道）+ connection/locale 转**可选探测**（apply 期 ctx.get 惰性判定，缺席降级不崩溃）", file: 'lib/client.js', line: 'L4357-L4361', necessity: 'consolidatable', incident: 'BUG-004', note: 'COMPAT-004 A3 已执行（DEC-026）：原三服务硬声明（L4350）使插件在无 connection/locale 的宿主上不挂载——现收敛为 slots 硬依赖 + ctx.get 惰性探测（宿主 cordis get() 对未提供服务返回 undefined，非抛错）；legacyApi 回退分支零改动（见 2.4）；残余收敛方向 = 轴① A1 版本矩阵（COMPAT-006）' },
    ...
    { face: 3, item: '3.4', kind: 'css-token', symbol: '宿主设计令牌：--dsw-alias-* 族（TK 令牌表 + NV_STYLE 全量 var(--dsw-alias-*, 兜底值) + color-mix 派生 L4304-4327）+ --dsw-shadow-lv2（弹窗阴影，带兜底）', file: 'lib/client.js', line: 'L680-693 / L4119 / L4254', necessity: 'required', note: '全文件去重令牌名恰 13 个 = alias 族 12 + shadow 族 1（REVIEW-COMPAT-002-R1 F2 修订：族外盲区关闭，见 regionLiterals.cssTokens）；F9：TK 表 L680-693 仅含 11 个去重 alias 名，第 12 个 --dsw-alias-bg-elevated 出自 L4254，--dsw-shadow-lv2 出自 L4119/L4254；兜底取值对齐 dsh-worktable styles.ts；令牌稳定化 TP-3 待验证' },
    ...
    { face: 3, item: '3.8', kind: 'dom-selector', symbol: "findSidebarEl() 侧栏探测：抽屉挂载点向上找 aside/nav 标签 + 宿主类名正则 /SidebarRoot|sidebar/i + document.querySelector('aside, nav, [class*=\"SidebarRoot\"], [class*=\"sidebar\"]') 兜底", file: 'lib/client.js', line: 'L2394-2404', necessity: 'required', incident: 'RISK-003', note: 'REVIEW-COMPAT-002-R1 F1 补录（§3 底稿未列、面 3 分布 7→8）：宿主侧栏类名/结构变更时契约面原为零信号；dsh-worktable 同款 findSidebar 约定；返回 null → 几何回退 left=280 默认值且不监听侧栏尺寸（L2755-2762 / L2770-2771，降级无白屏）' },
    ...
    // ── 面 6 — 版本与环境面（5 项）──
    { face: 6, item: '6.1', kind: 'version-policy', symbol: 'peerDependencies 全 *（cordis / schemastery / dsh-settings / dsh-home-paths / dsh-tools）', file: 'package.json', line: 'L41-47', necessity: 'improvable', note: '* = 不设限=不设防（无下限=隐式信任任意未来版本）；junction 共享 profile node_modules 保证闭包内一致；CI 依赖架构守卫（ci.yml L24-37）；四次事故都在 * 口径下发生' },
