...
export const hostContract = {
  ...
  revisions: [
    ...
    { task: 'CLEAN-007', scope: '**面 2/3 行号重基（UX-012 R1 备注批次 D-1/F2/F3/F4/F5/F6/F7/F9 落地的机械偏移）**：lib/client.js **5018 → 5198 行（+180）** ⇒ **18 项 / 31 处** `items[].line` 重基（2.3、2.4×6、2.5~2.13、3.1、3.2×2、3.3×3、3.4×3、3.5、3.6×2、3.8；**2.1/2.2 位于首个插入点之前不变**）。偏移分段：早期段 **+84**（F6 模态可访问性公共件 + `launcher.createSessionFor` 收口，均落在本段之前）；2.11 **+88**；3.8 **+148**；3.4 之 L4554 **+157**；2.3/2.12/3.4 之 L4689 **+165**；3.6 之 L3707 **+168**；2.13 **+175**。**取证口径（逐项，非估计值）**：① 范围**内容逐行等价**——旧 [a,b] ≡ 新 [a+Δ,b+Δ] 逐行比对且长度守恒（**31/31 全等价，0 处不等价**）；② 范围首/末**非空行原文**在**最终文件**中**唯一精确命中**（展开窗口 1~3 行，逐项记录新行号）。**未变**：items[] 计数与 face 分布、kind/necessity/symbol/`golden` 面计数、`regionLiterals`、`presetRowConfig[]` 与 item 5.3/5.4/5.6（BUG-007 交付面）**零改动**。伴随订正：`test/smoke.mjs` 注释内 5 串 `<item> L<n>` 陈旧副本（6 处）同 commit 同步为新区间（否则 COMPAT-015 F3 行号引用对账转红）' },
  ],
  faces: [
  ...
  items: [
    ...
    { face: 2, item: '2.3', kind: 'inject', symbol: "inject = ['slots']（硬依赖仅 UI 挂载通道）+ connection/locale 转**可选探测**（apply 期 ctx.get 惰性判定，缺席降级不崩溃）", file: 'lib/client.js', line: 'L5060-L5062', necessity: 'consolidatable', incident: 'BUG-004', note: '行号重基（COMPAT-005）。**note 口径（REVIEW-COMPAT-005-R1 F5）**：本 note 不内嵌「现 L…」行号——行号以本项 `line` 字段为唯一事实源（内嵌副本在后续重基后必然残留旧值，3.1/3.6/1.8 即实例）；起点取**声明行本体**（A3 说明注释块为前导，面 2 严格面要求边界为代码行）。COMPAT-004 A3 已执行（DEC-026）：原三服务硬声明使插件在无 connection/locale 的宿主上不挂载——现收敛为 slots 硬依赖 + ctx.get 惰性探测（宿主 cordis get() 对未提供服务返回 undefined，非抛错）；legacyApi 回退分支零改动（见 2.4）；残余收敛方向 = 轴① A1 版本矩阵（COMPAT-006）。**行号重基（UX-060）**：告警流式面新增段落在本项之前 ⇒ 机械偏移 +91，起止逐行实读复核（起点仍为声明行本体）' },
    { face: 2, item: '2.4', kind: 'api-call', symbol: 'connection.api（旧宿主 ≤0.1.x API 回退，全文件唯一特性检测点）', file: 'lib/client.js', line: 'L766 / L797 / L813 / L822 / L861 / L870', necessity: 'adapted', incident: 'BUG-004', note: 'makeHostApi 单点收口；代码级引用恰 1 处（EVD-077 审查复核 + smoke 断言）。行号重基（COMPAT-005；**note 不内嵌行号**——以本项 `line` 字段为唯一事实源；六处 legacyApi 回退返回逐点实读）' },
    { face: 2, item: '2.5', kind: 'remote-namespace', symbol: 'remote.settings（describe/update/mutate；位置参数 + expectedRevision acceptsUndefined 形状）', file: 'lib/client.js', line: 'L788-798', necessity: 'required', incident: 'BUG-004', note: '12 组形状映射之一（makeHostApi）。行号重基（COMPAT-005；行号以本项 `line` 字段为唯一事实源，note 不内嵌副本；终点取闭合 `},` 保持面内无缝隙）' },
    { face: 2, item: '2.6', kind: 'remote-namespace', symbol: 'remote.session（create/prompt/cancel；requestId 必填铸造 mintRequestId）', file: 'lib/client.js', line: 'L799-807', necessity: 'required', incident: 'BUG-004', note: '行号重基（COMPAT-005；行号以本项 `line` 字段为唯一事实源，note 不内嵌副本；起点由 BUG-004 注释块内行修正为 getter 行本体）' },
    { face: 2, item: '2.7', kind: 'remote-namespace', symbol: 'remote.workspace + workspaces 快照服务（list=getSnapshot/subscribe、phase pending→ready、awaitWorkspacesReady 订阅等待）', file: 'lib/client.js', line: 'L815-842', necessity: 'required', incident: 'BUG-004', note: '行号重基（COMPAT-005；行号以本项 `line` 字段为唯一事实源，note 不内嵌副本；终点取 workspace getter 闭合行，不侵入 2.8 的 getter）' },
    { face: 2, item: '2.8', kind: 'remote-namespace', symbol: 'remote.directoryPicker（pick string→{path} 包装 / createDirectory / list）', file: 'lib/client.js', line: 'L843-862', necessity: 'required', incident: 'BUG-004', note: '行号重基（COMPAT-005；行号以本项 `line` 字段为唯一事实源，note 不内嵌副本）' },
    { face: 2, item: '2.9', kind: 'remote-namespace', symbol: 'remote.agentPresets（select 两参位置）', file: 'lib/client.js', line: 'L863-872', necessity: 'required', incident: 'BUG-004', note: '行号重基（COMPAT-005；行号以本项 `line` 字段为唯一事实源，note 不内嵌副本；起点由前项注释尾行修正为 getter 行本体）' },
    { face: 2, item: '2.10', kind: 'reactive-store', symbol: 'sessions.list ObservableSnapshot（getSnapshot/subscribe 形状校验，makeSessionsHookReactive 内前段）', file: 'lib/client.js', line: 'L1445-1509', necessity: 'required', incident: 'BUG-005', note: '会话状态镜像（零轮询）。行号重基（COMPAT-005；整函数跨度不变——行号以本项 `line` 字段为唯一事实源，note 不内嵌副本）。**行号重基（UX-060）**：告警流式面 i18n/常量新增段落在本项之前 ⇒ 机械偏移 +9，整函数跨度不变（逐行实读复核）' },
    { face: 2, item: '2.11', kind: 'api-call', symbol: 'launcher.sessions（sessions.open(sessionId) 打开/切换会话）', file: 'lib/client.js', line: 'L1537-1564', necessity: 'required', note: '行号重基（COMPAT-005；起点回到控制器对象声明行 `const launcher = {`（原值起点落在无关行，⑫b 判据①③自 004-R1 起即报偏移）——终点取 open() 闭合行，⑤b 构造闭合落在范围内；行号以本项 `line` 字段为唯一事实源）。**行号重基（UX-060）**：告警流式面新增段落在本项之前 ⇒ 机械偏移 +9，起点仍为控制器对象声明行、终点仍为 open() 闭合行（逐行实读复核）' },
    { face: 2, item: '2.12', kind: 'event', symbol: "ctx.on('internal/service')（单一监听器三消费：locale 重快照 + launcher 重 setup + hook.refresh）", file: 'lib/client.js', line: 'L5090-5097', necessity: 'required', incident: 'BUG-004+BUG-005', note: 'cordis 4.x 事件（EVD-077 实证）；服务缺席↔到位自愈。**描述订正（COMPAT-013 F-4，只读 client.js 逐行实读）**：原记「单一监听器**双消费**（launcher 重 setup + hook.refresh）」已 stale——REVIEW-COMPAT-004-R1 F2 为 locale「后到」自愈在同一监听器内加了 `locale` 分支（`snapshotLocale(ctx)` + `rerender()`），实为 **3 消费者**（locale 分支 / launcher 重 setup / hook.refresh）。**行号重基（COMPAT-005）**：行号以本项 `line` 字段为唯一事实源（note 不内嵌行号副本）；起点取声明行 `const offServiceEvent = …` 本体（旧值起点落在无关 UX-026 注释行上，⑫b 判据①③自 004-R1 起即报偏移）。**行号重基（UX-060）**：告警流式面新增段落在本项之前 ⇒ 机械偏移 +91，起点仍为 `const offServiceEvent = …` 声明行本体（逐行实读复核）' },
    { face: 2, item: '2.13', kind: 'slot', symbol: "ctx.slots.inject('settings.section'|'sidebar.footer.action'|'shell.overlay') + ctx.slots.register({name,id,order,label}) ×6 注册", file: 'lib/client.js', line: 'L5127-5159', necessity: 'required', note: 'UI 挂载槽位名约定，无替代挂载通道；槽位名稳定性 TP-2 待验证。**行号重基（COMPAT-005）**：行号以本项 `line` 字段为唯一事实源（note 不内嵌行号副本）；6 处 `ctx.slots.inject(...)` 与 6 处 `ctx.slots.register(...)` 全部落在范围内（judge ⑤a 计数 6 ≡ 6），终点取末次 register 的 `))` 闭合行。**N3 行为边界（REVIEW-COMPAT-004-R2，COMPAT-005 登记）**：宿主侧槽位**标签**（本项 label()）的刷新依赖「宿主在 locale 变化后重渲染自身 shell」——本侧已保证**必要条件**（label 为渲染期惰性取值，smoke ④b 实测 zh⇒en），但「宿主会重渲染」这一前提**在本仓无证据（前提未证实）**：若宿主不重渲染 shell，宿主侧标签滞后到下一次自然重渲染（自有 React 组件经 store 空通知已即时刷新）。**N4 已知边界（同轮，COMPAT-005 登记）**：locale 服务**撤离**时保持最后一次快照（不强行回退 zh——避免宿主热重载期间标签抖动），撤离同样触发一次空重渲染（无害）。**行号重基（UX-060）**：告警流式面新增段落在本项之前 ⇒ 机械偏移 +91；6 处 `ctx.slots.inject(...)` / `ctx.slots.register(...)` 仍全部落在范围内（⑤a 计数 6 ≡ 6），终点仍为末次 register 的 `))` 闭合行（逐行实读复核）' },
    ...
    { face: 3, item: '3.1', kind: 'dom-selector', symbol: "document.querySelectorAll('[data-phase]')（findConversationRoot：phase=active 优先 + children≥2 + 排除 TEXTAREA/INPUT）", file: 'lib/client.js', line: 'L1071-1079', necessity: 'required', incident: 'RISK-003', note: '宿主无官方布局 API；dsh-worktable 同款约定；null → 不做布局降级（实读同函数内 null 分支）。行号重基（COMPAT-005）。**note 口径（REVIEW-COMPAT-005-R1 F5）**：不内嵌「现 L…」行号（行号以 `line` 字段为唯一事实源）；起于 JSDoc `/**` ——非严格面合法注释起段，由 smoke ⑫d golden 锁定。**行号重基（UX-060）**：告警流式面新增段落在本项之前 ⇒ 机械偏移 +9，起点仍为 JSDoc `/**` 注释行（非严格面合法注释起段计数 2 不变）（逐行实读复核）' },
    { face: 3, item: '3.2', kind: 'dom-structure', symbol: 'root.children[0]=header / root.children[1]=viewArea 结构假设', file: 'lib/client.js', line: 'L1134-1135 / L1218', necessity: 'required', incident: 'RISK-003', note: 'undefined 检查降级（同函数内早退返回 false）。行号重基（COMPAT-005；行号以本项 `line` 字段为唯一事实源，note 不内嵌副本）' },
    { face: 3, item: '3.3', kind: 'layout-technique', symbol: 'margin 挤压（marginLeft/Right/Top + width 双压——UX-018 修复：margin 不触发宿主响应式重排，需显式 width）', file: 'lib/client.js', line: 'L1146-1149 / L1226-1231 / L1283-1285', necessity: 'required', incident: 'RISK-003 + UX-018', note: 'dsh-worktable 同款；saved* 原值恢复机制完备 + 让位观察器（同文件后段）。行号重基（COMPAT-005；行号以本项 `line` 字段为唯一事实源，note 不内嵌副本）' },
    { face: 3, item: '3.4', kind: 'css-token', symbol: '宿主设计令牌：--dsw-alias-* 族（TK 令牌表 + NV_STYLE 全量 var(--dsw-alias-*, 兜底值) + color-mix 派生）+ --dsw-shadow-lv2（弹窗阴影，带兜底）', file: 'lib/client.js', line: 'L921-934 / L4711 / L4854', necessity: 'required', note: '全文件去重令牌名恰 13 个 = alias 族 12 + shadow 族 1（REVIEW-COMPAT-002-R1 F2 修订：族外盲区关闭，见 regionLiterals.cssTokens）；TK 表（见本项 `line` 字段首段）仅含 11 个去重 alias 名，第 12 个 `--dsw-alias-bg-elevated` 与 `--dsw-shadow-lv2` 出自 NV_STYLE 的 `.nv-cmodal` 段（`--dsw-shadow-lv2` 另见于 `.nv-modal` 段）；兜底取值对齐 dsh-worktable styles.ts；令牌稳定化 TP-3 待验证。**行号重基（COMPAT-005；行号以本项 `line` 字段为唯一事实源，note 不内嵌副本）**：新增 D1 面板样式未引入族外令牌' },
    { face: 3, item: '3.5', kind: 'dom-interop', symbol: ".ta_splitClose 类名点击 + 'dsh:split-claim' CustomEvent（与 dsh-worktable 跨插件分栏互操作）", file: 'lib/client.js', line: 'L1121-1133', necessity: 'optional', note: '生态约定非宿主官方；try/catch 包裹 + claim 监听让位（让位段 = `apply` 内 `claimHandler` 段，与 2.12 的 `offServiceEvent` 同区）。**行号重基（COMPAT-005；行号以本项 `line` 字段为唯一事实源，note 不内嵌副本）**；起于互操作说明前的空行段——非严格面合法起段，由 smoke ⑫d golden 锁定（恰 2 处之一，另一处 = 3.1 的 JSDoc）。**行号重基（UX-060）**：告警流式面新增段落在本项之前 ⇒ 机械偏移 +9，起点仍为空行段、终点仍为 `.ta_splitClose` 点击行（逐行实读复核）' },
    { face: 3, item: '3.6', kind: 'dom-observer', symbol: "ResizeObserver + MutationObserver(attributeFilter:['data-phase'])（DOM/尺寸变化跟随重锚定）", file: 'lib/client.js', line: 'L1160-1184 / L3875', necessity: 'required', incident: 'RISK-003', note: '探测的是变化后的自救，不是宿主约定的变更探测。行号重基（COMPAT-005）。**note 口径（REVIEW-COMPAT-005-R1 F5）**：不内嵌「现 L…」行号（行号以 `line` 字段为唯一事实源——原内嵌副本为字段旧值，残留即成 stale）' },
    ...
    { face: 3, item: '3.8', kind: 'dom-selector', symbol: "findSidebarEl() 侧栏探测：抽屉挂载点向上找 aside/nav 标签 + 宿主类名正则 /SidebarRoot|sidebar/i + document.querySelector('aside, nav, [class*=\"SidebarRoot\"], [class*=\"sidebar\"]') 兜底", file: 'lib/client.js', line: 'L2977-2987', necessity: 'required', incident: 'RISK-003', note: 'REVIEW-COMPAT-002-R1 F1 补录（§3 底稿未列、面 3 分布 7→8）：宿主侧栏类名/结构变更时契约面原为零信号；dsh-worktable 同款 findSidebar 约定；返回 null → 几何回退 left=280 默认值且不监听侧栏尺寸（降级无白屏）。行号重基（COMPAT-005；行号以本项 `line` 字段为唯一事实源，note 不内嵌副本）' },
