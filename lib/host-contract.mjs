...
export const hostContract = {
  ...
  items: [
    ...
    { face: 2, item: '2.3', kind: 'inject', symbol: "inject = ['slots']（硬依赖仅 UI 挂载通道）+ connection/locale 转**可选探测**（apply 期 ctx.get 惰性判定，缺席降级不崩溃）", file: 'lib/client.js', line: 'L4374-L4377', necessity: 'consolidatable', incident: 'BUG-004', note: 'COMPAT-004 A3 已执行（DEC-026）：原三服务硬声明（L4367）使插件在无 connection/locale 的宿主上不挂载——现收敛为 slots 硬依赖 + ctx.get 惰性探测（宿主 cordis get() 对未提供服务返回 undefined，非抛错）；legacyApi 回退分支零改动（见 2.4）；残余收敛方向 = 轴① A1 版本矩阵（COMPAT-006）' },
    { face: 2, item: '2.4', kind: 'api-call', symbol: 'connection.api（旧宿主 ≤0.1.x API 回退，全文件唯一特性检测点）', file: 'lib/client.js', line: 'L549 / L573 / L589 / L598 / L637 / L646', necessity: 'adapted', incident: 'BUG-004', note: 'makeHostApi 单点收口；代码级引用恰 1 处（EVD-077 审查复核 + smoke 断言）' },
    { face: 2, item: '2.5', kind: 'remote-namespace', symbol: 'remote.settings（describe/update/mutate；位置参数 + expectedRevision acceptsUndefined 形状）', file: 'lib/client.js', line: 'L564-574', necessity: 'required', incident: 'BUG-004', note: '12 组形状映射之一（makeHostApi）' },
    { face: 2, item: '2.6', kind: 'remote-namespace', symbol: 'remote.session（create/prompt/cancel；requestId 必填铸造 L506-514）', file: 'lib/client.js', line: 'L575-590', necessity: 'required', incident: 'BUG-004' },
    { face: 2, item: '2.7', kind: 'remote-namespace', symbol: 'remote.workspace + workspaces 快照服务（list=getSnapshot/subscribe、phase pending→ready、awaitWorkspacesReady 订阅等待 L516-542）', file: 'lib/client.js', line: 'L591-618', necessity: 'required', incident: 'BUG-004' },
    { face: 2, item: '2.8', kind: 'remote-namespace', symbol: 'remote.directoryPicker（pick string→{path} 包装 L627 / createDirectory / list）', file: 'lib/client.js', line: 'L619-638', necessity: 'required', incident: 'BUG-004' },
    { face: 2, item: '2.9', kind: 'remote-namespace', symbol: 'remote.agentPresets（select 两参位置）', file: 'lib/client.js', line: 'L639-647', necessity: 'required', incident: 'BUG-004' },
    { face: 2, item: '2.10', kind: 'reactive-store', symbol: 'sessions.list ObservableSnapshot（getSnapshot/subscribe 形状校验 L1218-1221，makeSessionsHookReactive）', file: 'lib/client.js', line: 'L1212-1276', necessity: 'required', incident: 'BUG-005', note: '会话状态镜像（零轮询）' },
    { face: 2, item: '2.11', kind: 'api-call', symbol: 'launcher.sessions（sessions.open(sessionId) 打开/切换会话）', file: 'lib/client.js', line: 'L1317-1327', necessity: 'required' },
    { face: 2, item: '2.12', kind: 'event', symbol: "ctx.on('internal/service')（单一监听器三消费：L4404 locale 重快照 + L4406 launcher 重 setup + L4407 hook.refresh）", file: 'lib/client.js', line: 'L4403-4408', necessity: 'required', incident: 'BUG-004+BUG-005', note: 'cordis 4.x 事件（EVD-077 实证）；服务缺席↔到位自愈。**描述订正（COMPAT-013 F-4，只读 client.js 逐行实读）**：原记「单一监听器**双消费**（launcher 重 setup + hook.refresh）」已 stale——REVIEW-COMPAT-004-R1 F2 为 locale「后到」自愈在同一监听器内加了 `locale` 分支（L4404 `snapshotLocale + rerender`），实为 **3 消费者**（L4404 / L4406 / L4407）' },
    { face: 2, item: '2.13', kind: 'slot', symbol: "ctx.slots.inject('settings.section'|'sidebar.footer.action'|'shell.overlay') + ctx.slots.register({name,id,order,label}) ×6 注册", file: 'lib/client.js', line: 'L4429-4461', necessity: 'required', note: 'UI 挂载槽位名约定，无替代挂载通道；槽位名稳定性 TP-2 待验证' },

    // ── 面 3 — 宿主 DOM/前端约定面（8 项）──
    { face: 3, item: '3.1', kind: 'dom-selector', symbol: "document.querySelectorAll('[data-phase]')（findConversationRoot：phase=active 优先 + children≥2 + 排除 TEXTAREA/INPUT）", file: 'lib/client.js', line: 'L838-846', necessity: 'required', incident: 'RISK-003', note: '宿主无官方布局 API；dsh-worktable 同款约定；null → 不做布局降级（L908-909）' },
    { face: 3, item: '3.2', kind: 'dom-structure', symbol: 'root.children[0]=header / root.children[1]=viewArea 结构假设', file: 'lib/client.js', line: 'L910-911 / L994', necessity: 'required', incident: 'RISK-003', note: 'undefined 检查降级（L912 return false）' },
    { face: 3, item: '3.3', kind: 'layout-technique', symbol: 'margin 挤压（marginLeft/Right/Top + width 双压——UX-018 修复：margin 不触发宿主响应式重排，需显式 width）', file: 'lib/client.js', line: 'L922-925 / L1003-1014 / L1060-1062', necessity: 'required', incident: 'RISK-003 + UX-018', note: 'dsh-worktable 同款；saved* 原值恢复机制完备 + 让位观察器（L960-972）' },
    { face: 3, item: '3.4', kind: 'css-token', symbol: '宿主设计令牌：--dsw-alias-* 族（TK 令牌表 + NV_STYLE 全量 var(--dsw-alias-*, 兜底值) + color-mix 派生 L4321-4344）+ --dsw-shadow-lv2（弹窗阴影，带兜底）', file: 'lib/client.js', line: 'L697-710 / L4136 / L4271', necessity: 'required', note: '全文件去重令牌名恰 13 个 = alias 族 12 + shadow 族 1（REVIEW-COMPAT-002-R1 F2 修订：族外盲区关闭，见 regionLiterals.cssTokens）；F9：TK 表 L697-710 仅含 11 个去重 alias 名，第 12 个 --dsw-alias-bg-elevated 出自 L4271，--dsw-shadow-lv2 出自 L4136/L4271；兜底取值对齐 dsh-worktable styles.ts；令牌稳定化 TP-3 待验证' },
    { face: 3, item: '3.5', kind: 'dom-interop', symbol: ".ta_splitClose 类名点击 + 'dsh:split-claim' CustomEvent（与 dsh-worktable 跨插件分栏互操作）", file: 'lib/client.js', line: 'L896-907', necessity: 'optional', note: '生态约定非宿主官方；try/catch 包裹 + claim 监听让位（**L4411-4424**）。**行段订正（COMPAT-013 F-4，只读 client.js 逐行实读）**：原记 `L4406-4417` 为 stale——真实 claim 让位段 = L4411-4424（L4413 `let claimHandler = null`、L4422 `window.addEventListener(\'dsh:split-claim\', claimHandler)`、L4424 `} catch`）；旧值起点偏差 −5 且**截断处未含 addEventListener 注册行**（L4422 落在旧区间外）' },
    { face: 3, item: '3.6', kind: 'dom-observer', symbol: "ResizeObserver + MutationObserver(attributeFilter:['data-phase'])（DOM/尺寸变化跟随重锚定）", file: 'lib/client.js', line: 'L934-957 / L2793', necessity: 'required', incident: 'RISK-003', note: '探测的是变化后的自救，不是宿主约定的变更探测' },
    { face: 3, item: '3.7', kind: 'storage-key', symbol: "localStorage 键：dsh.novel.split.v1 / dsh.novel.defaults.v1 / dsh.novel.order.v1", file: 'lib/client.js', line: 'L744-746 等', necessity: 'own', note: '自有口径统一=1 项（REVIEW-COMPAT-001-R1 F10）：插件自有点、无宿主耦合，列出为完整性' },
    { face: 3, item: '3.8', kind: 'dom-selector', symbol: "findSidebarEl() 侧栏探测：抽屉挂载点向上找 aside/nav 标签 + 宿主类名正则 /SidebarRoot|sidebar/i + document.querySelector('aside, nav, [class*=\"SidebarRoot\"], [class*=\"sidebar\"]') 兜底", file: 'lib/client.js', line: 'L2411-2421', necessity: 'required', incident: 'RISK-003', note: 'REVIEW-COMPAT-002-R1 F1 补录（§3 底稿未列、面 3 分布 7→8）：宿主侧栏类名/结构变更时契约面原为零信号；dsh-worktable 同款 findSidebar 约定；返回 null → 几何回退 left=280 默认值且不监听侧栏尺寸（L2772-2779 / L2787-2788，降级无白屏）' },
