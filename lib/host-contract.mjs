...
export const hostContract = {
  ...
  items: [
    ...
    { face: 2, item: '2.12', kind: 'event', symbol: "ctx.on('internal/service')（单一监听器双消费：launcher 重 setup + hook.refresh）", file: 'lib/client.js', line: 'L4391-4397', necessity: 'required', incident: 'BUG-004+BUG-005', note: 'cordis 4.x 事件（EVD-077 实证）；服务缺席↔到位自愈' },
    { face: 2, item: '2.13', kind: 'slot', symbol: "ctx.slots.inject('settings.section'|'sidebar.footer.action'|'shell.overlay') + ctx.slots.register({name,id,order,label}) ×6 注册", file: 'lib/client.js', line: 'L4422-4454', necessity: 'required', note: 'UI 挂载槽位名约定，无替代挂载通道；槽位名稳定性 TP-2 待验证' },
