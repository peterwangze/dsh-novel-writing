...
export const hostContract = {
  ...
  items: [
    ...
    { face: 3, item: '3.6', kind: 'dom-observer', symbol: "ResizeObserver + MutationObserver(attributeFilter:['data-phase'])（DOM/尺寸变化跟随重锚定）", file: 'lib/client.js', line: 'L1164-1188 / L3926', necessity: 'required', incident: 'RISK-003', note: '探测的是变化后的自救，不是宿主约定的变更探测。行号重基（COMPAT-005）。**note 口径（REVIEW-COMPAT-005-R1 F5）**：不内嵌「现 L…」行号（行号以 `line` 字段为唯一事实源——原内嵌副本为字段旧值，残留即成 stale）' },
    ...
    { face: 3, item: '3.8', kind: 'dom-selector', symbol: "findSidebarEl() 侧栏探测：抽屉挂载点向上找 aside/nav 标签 + 宿主类名正则 /SidebarRoot|sidebar/i + document.querySelector('aside, nav, [class*=\"SidebarRoot\"], [class*=\"sidebar\"]') 兜底", file: 'lib/client.js', line: 'L3023-L3033', necessity: 'required', incident: 'RISK-003', note: 'REVIEW-COMPAT-002-R1 F1 补录（§3 底稿未列、面 3 分布 7→8）：宿主侧栏类名/结构变更时契约面原为零信号；dsh-worktable 同款 findSidebar 约定；返回 null → 几何回退 left=280 默认值且不监听侧栏尺寸（降级无白屏）。行号重基（COMPAT-005；行号以本项 `line` 字段为唯一事实源，note 不内嵌副本）' },
