    ...
    function StudioView(props) {
      ...
      return el('div', { style: { display: 'flex', flexDirection: 'column', height: '100%', padding: '12px', boxSizing: 'border-box' } },
        el('div', { style: { display: 'flex', gap: '10px', alignItems: 'center', marginBottom: '8px' } },
          el('select', { style: input, value: selectedId ?? '', onChange: (e) => store.set({ selected: e.target.value }) },
            novels.map((n) => el('option', { key: n.id, value: n.id }, `${n.title}（${n.stage ?? '—'} · ${n.totalChapters} 章）`))),
