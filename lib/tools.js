...
export function apply(ctx) {
  ...
  ctx.tools.register(defineTool({
          ...
          novels: {
            ...
            items: {
              ...
              properties: {
                ...
                stage: { type: 'string' }, totalChapters: { type: 'integer' },
    ...
    async execute() { return service.listNovels() },
  }))
  ...
  ctx.tools.register(defineTool({
    ...
    async execute(args) {
      ...
      return {
        ...
        latestAiPath: detail.state.guardrails?.latest_ai_path ?? null,
        latestDrift: detail.state.guardrails?.latest_drift_score ?? null,
  ...
  ctx.tools.register(defineTool({
    ...
    async execute(args) {
      ...
      const state = service.updateState(args.novel, patch)
  ...
  ctx.tools.register(defineTool({
    ...
    render(_args, value) {
      const g = value.gate
      if (g === null || g === undefined) return text(`已保存：${value.words} 字（无看护卡，门禁未启用）`)
  ...
  ctx.tools.register(defineTool({
    ...
    output: {
      schema: {
        type: 'object', additionalProperties: false,
        properties: { available: { type: 'boolean' }, coverage: { type: 'number' }, drift: { type: 'number' }, passed: { type: 'boolean' }, scenes: { type: 'array', items: { type: 'json' } }, bannedHits: { type: 'array', items: { type: 'string' } }, note: { type: 'string' } },
      },
    },
    render(_args, value) {
      if (value.available !== true) return text('无看护卡（context card 尚未生成），门禁未启用。')
    ...
    async execute(args) {
      ...
      return service.gateCheck(args.novel, args.chapter, content)
    },
  }))

  ctx.tools.register(defineTool({
    name: 'novel_review_submit',
    description: '提交一章的审查结论（质量审查阶段）：记录分数、硬门禁判定与发现列表；找到问题必须引用原文。无发现的"通过"视为无效审查；分数 <100 视为未通过（review 门禁零容忍）。',
    parameters: {
      novel: { type: 'string', required: true },
      chapter: { type: 'integer', required: true },
      verdict: { type: 'string', enum: ['pass', 'fail'], required: true },
      score: { type: 'integer', required: true, description: '9 维满分 100 制得分' },
      findings: { type: 'array', items: { type: 'string' }, required: true, description: '每条含章节内引用与严重程度（阻断/警告/建议）；至少 2 条具体发现' },
      hardGates: { type: 'array', items: { type: 'string' }, description: '未通过的硬门禁名称（[] 表示全部通过）' },
    },
    output: {
      schema: {
        type: 'object', additionalProperties: false,
        properties: { ok: { type: 'boolean' }, score: { type: 'integer' }, verdict: { type: 'string' }, reviewer: { type: 'string' } },
      },
    },
    render(_args, value) {
    ...
    async execute(args) {
      if (args.findings.length < 2) throw new Error('review invalid: 至少 2 条具体发现（无发现的"通过"视为无效审查）')
      const numStr = String(args.chapter).padStart(3, '0')
      const meta = service.readMeta(args.novel)
      meta.chapters = meta.chapters ?? {}
      meta.chapters[numStr] = {
        ...(meta.chapters[numStr] ?? {}),
        reviewScore: args.score,
        reviewVerdict: args.verdict,
        reviewFindings: args.findings,
        reviewHardGates: args.hardGates ?? [],
        reviewAt: new Date().toISOString(),
      }
      service.writeMeta(args.novel, meta)
      const report = {
        chapter: args.chapter, verdict: args.verdict, score: args.score,
        findings: args.findings, hardGates: args.hardGates ?? [],
        at: new Date().toISOString(),
      }
      const dir = service.projectDir(args.novel) + '/10-reviews/quality-reports'
      const { mkdirSync, writeFileSync } = await import('node:fs')
      const { join } = await import('node:path')
      mkdirSync(dir, { recursive: true })
      writeFileSync(join(dir, `chapter-${numStr}-review.json`), JSON.stringify(report, null, 2) + '\n', 'utf8')
      service.pushVersion(`review:${args.novel}:${numStr}`)
      return { ok: true, score: args.score, verdict: args.verdict, reviewer: 'coordinator' }
    },
  }))

  ctx.tools.register(defineTool({
    name: 'novel_data_ingest',
    description: '平台数据入库并触发信号检测：records 为 {date, 完读率/readRate, 读完率/finishRate, 追读/retention, 日增收藏/dailyCollections, 收藏/collections, 收益/revenue, 评论/comments, dropRate?} 数组。入库即返回数据信号（完读率低/章节流失/追读下降/收藏停滞/收益下滑）。',
    parameters: {
      novel: { type: 'string', required: true },
      records: { type: 'array', items: { type: 'json' }, required: true, description: '指标记录数组（每项必须含 date: YYYY-MM-DD）' },
    },
    output: {
      schema: {
        type: 'object', additionalProperties: false,
  ...
  ctx.tools.register(defineTool({
    ...
    async execute(args) {
      const rel = service.novelDetail(args.novel).state.guardrails?.release_allowed === true
      if (rel !== true) throw new Error('release_allowed=false：AI 合规门禁未通过，禁止进入发布流程（先完成 human_ai_collaboration 评估）')
