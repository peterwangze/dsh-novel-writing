 ...
 * 数据入库即算信号并给出建议行动。
 *
 * 硬依赖：`tools`（模型工具注册表，DSH 核心提供）——Cordis 规定 ctx 服务
 * 属性访问必须先在插件声明 inject，本模块导出 `inject = ['tools']`。
 *
...
/** 硬依赖：tools（模型工具注册表，DSH 核心提供）。novel-writing 服务为可选依赖（ctx.get 探测，宿主 bundle 行未安装时本行静默激活、注册 0 个工具），不得加入 inject——加入会使未装 bundle 的预设无法挂载。 */
...
export function apply(ctx) {
  ...
  ctx.tools.register(defineTool({
    ...
    parameters: {
      ...
      latestAiPath: { type: 'string', enum: ['A', 'B', 'C'], description: 'AI 参与路径评级（human-ai-collaboration 阶段产出）' },
    },
    output: {
      schema: {
        type: 'object', additionalProperties: false,
