...
class NovelWritingService extends Service {
  ...
  /**
   * 章节名提取（UX-015②）：取章节文件首个 `# ` 标题行，剥掉「第N章」前缀
   * （支持中文/阿拉伯数码，「# 第1章 关于你明天的死亡，规则如下」→
   * 「关于你明天的死亡，规则如下」）；无标题行或剥后为空 → ''。
   */
  chapterNameOf(content) {
    const first = content.split('\n').find((l) => l.startsWith('# '))
    if (first === undefined) return ''
    return first.slice(2).trim().replace(/^第[0-9０-９一二三四五六七八九十百千零万两]+章\s*/, '').trim()
  }
