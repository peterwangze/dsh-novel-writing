 ...
 *
 * COMPAT-004（宿主边界层）：本文件的**全部宿主调用**经 `./host-boundary.js` 收口（宿主包导入 / settings
 * 注册读取 / webServer 路由挂载 / $DSH_HOME 预设路径 / Service 基类与服务名 / 日志·事件·生命周期 +
 * apply 期兼容自检）——本文件零直连宿主 API（smoke 收口断言机检）。宿主升级时服务端适配只改边界层。
 *
 * @module dsh-novel-writing
 */
import {
  Service, z, SERVICE_NAME, hostInject,
  registerSettings, readSettings, getWebServer, registerRoute, onLifecycle,
  loggerOf, logWarn, emitChanged, presetRoot, presetDir, selfCheckHost, detectHostCapabilities,
...
/** 硬依赖：settings（配置节）。webServer / timer 可选。声明来自边界层（契约 1.5；F6：**副本**非共享引用）。 */
export const inject = [...hostInject] // F6 副本：宿主/消费方就地改写（push/splice）不再污染边界 hostInject（探测判据 1.5 的输入）

/** 设置命名空间：普通字符串，须匹配 dsh-settings 的 /^[a-z][a-z0-9-]*$/（register/get 自带校验）。 */
const NS = 'novel-writing'
...
class NovelWritingService extends Service {
  ...
  pushVersion(detail) {
    this.version += 1
    try {
      emitChanged(this.ctx, { version: this.version, detail: detail ?? null })
    } catch (error) {
  ...
  registerHttp(webServer) {
    ...
    api('preset-sync', async () => {
      this.ensurePreset()
      return { ok: true, dir: presetDir(PRESET_ID) }
    })

    // COMPAT-005 D1：诊断面板的数据源路由——把 COMPAT-004 的 `detectHostCapabilities` 报告
    // （apply 期已算的同一函数）经既有 webServer 路由模式暴露给客户端设置页「诊断」区。
    // 口径纪律（P-02/BC-05）：①只读（探测纯同步、零副作用、无网络/布局等待）；②载荷仅布尔/名称/
    // 版本——**不含 file/line/paths**（宿主/仓库文件路径一律不出网关）；③探测抛错如实返回
    // `{ ok:false, error }`（不伪造绿也不冒泡 500）；④与 [nv-compat] 告警**同源同函数**，
    // 客户端面板不另立事实源（报告项 = 契约声明面的运行时投影）。
    api('compat', async () => {
      const ctxNow = this.ctx
      if (ctxNow === undefined || ctxNow === null) return { ok: false, error: 'probe-unavailable' }
      let report
      try {
        report = detectHostCapabilities(ctxNow)
      } catch (error) {
      ...
      return {
        ok: true,
        report,
        // 契约声明面投影（items 只取 item/face/kind/necessity——symbol/file/line 不出网关）：
        // 面板据此展示「已实现面 vs 未实现面」的**数量摘要**，不伪造未实现面的状态。
        contract: {
          task: hostContract.task,
          schemaVersion: hostContract.schemaVersion,
          revision: Array.isArray(hostContract.revisions) && hostContract.revisions.length > 0
            ? hostContract.revisions.at(-1).task
            : hostContract.task,
          faces: hostContract.faces.map((f) => ({ id: f.id, name: f.name })),
          items: hostContract.items.map((it) => ({ item: it.item, face: it.face, kind: it.kind, necessity: it.necessity })),
        },
      }
    })
  }
...
export function apply(ctx) {
  ...
  registerSettings(ctx, NS, Config)

  service.ensurePreset()

  const webServer = getWebServer(ctx)
  if (webServer !== undefined) {
    onLifecycle(ctx, () => service.registerHttp(webServer), 'dsh-novel-writing: http routes')
  } else {
    logWarn(ctx, 'dsh-novel-writing: no webServer mounted; browser studio API unavailable')
  }
  service.ctx = ctx
  // COMPAT-004 D2（轴④）：apply 期结构化兼容自检——探测全同步（零网络/零布局等待），
  // 缺面单次 `[nv-compat]` 告警（含域名/方法名/服务名，仅布尔·名称·版本），不阻断加载。
  selfCheckHost(ctx)
  return service
}
