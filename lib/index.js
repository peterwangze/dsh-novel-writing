...
class NovelWritingService extends Service {
  ...
  registerHttp(webServer) {
    ...
    // 版本——**不含 file/line/paths**（宿主/仓库文件路径一律不出网关；自由文本字段一律降为**枚举码**，
    // 见 host-boundary `FACE1_UNPROBED_CODES` / `FACES_NOT_COVERED_AT_APPLY`——REVIEW-COMPAT-005-R1 F2）；
    // ③探测抛错如实返回 `{ ok:false, error }`（不伪造绿也不冒泡 500）；④与 [nv-compat] 告警**同源同函数**，
    // 客户端面板不另立事实源（报告项 = 契约声明面的运行时投影）；⑤契约投影经边界层 `contractProjection()`
    // 单点产出（**产品代码不直接 import 契约模块**——依赖方向 = 产品 → 边界 → 契约，R1 F7）。
    api('compat', async () => {
      ...
      return {
        ok: true,
        report,
        // 契约声明面投影（items 只取 item/face/kind/necessity——symbol/file/line 不出网关；revision 复用
        // 契约修订史末项派生，单一派生点在边界层）：
