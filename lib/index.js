...
class NovelWritingService extends Service {
  ...
  createProject(name, title) {
    ...
    if (/[<>:"|?*\\/]/.test(name) || name === '.' || name === '..') throw new Error(`invalid novel name: ${name}`)
    const dir = this.novelDir(name)
    if (existsSync(dir)) throw new Error(`novel already exists: ${name}`)
    mkdirSync(join(dir, PROJECT_DIR), { recursive: true })
    const state = this.updateState(name, { project_info: { title: typeof title === 'string' && title !== '' ? title : name } })
    this.pushVersion(`create:${name}`)
    return { id: name, title: state.project_info.title, state }
  }
