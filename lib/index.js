...
class NovelWritingService extends Service {
  ...
  ensurePreset() {
    try {
      ...
      // 预设目录由插件拥有：版本变化时整体重建（先删后拷），旧版本文件不残留。
      if (existsSync(userDir)) rmSync(userDir, { recursive: true, force: true })
      cpSync(pkgDir, userDir, { recursive: true })
  ...
  listNovels() {
    ...
    try {
      ...
      return { root, novels: [], pollMs: this.cfg().pollMs ?? 2000 }
    }
    const novels = []
  ...
  createProject(name, title) {
    ...
    return { id: clean, title: state.project_info.title, state }
  }
  ...
  ingestData(novelId, records) {
    ...
    this.atomicWrite(this.dataFilePath(novelId), JSON.stringify(existing, null, 2) + '\n')
